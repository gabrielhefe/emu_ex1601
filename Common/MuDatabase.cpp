...
void MuDatabaseConnection::DoPrepareStatements()
{
	...
	PrepareStatement(QUERY_MUDATABASE_ACCOUNT_DATA_CREATE, "INSERT IGNORE INTO account_data (account_id, current_ip, current_mac, current_diskserial, current_server, expanded_warehouse, expanded_warehouse_time) VALUES (?, ?, ?, ?, ?, 1, 0)", CONNECTION_ASYNC);
	...
	PrepareStatement(ACCOUNT_WAREHOUSE_DATA_REPLACE, "UPDATE IGNORE account_warehouse SET account_id = ?, money = ?, password = ?", CONNECTION_ASYNC);
	...
	PrepareStatement(CHARACTER_SELECT_INFO, "SELECT c.guid, c.slot, c.authority, c.race, c.name, c.level, c.level_master, c.experience, c.experience_master, "
		...
		"c.goblin_points, c.kick_time, c.account_id, c.post_count, c.post_day, c.post_month, c.ruud_money, c.level_majestic, c.experience_majestic, c.points_majestic, UNIX_TIMESTAMP(c.create_date), c.hunting_log_visible, c.reset "
	...
	PrepareStatement(CHARACTER_UPDATE_INFO, "UPDATE character_info SET "
			...
			"goblin_points = ?, last_use = ?, kick_time = ?, post_count = ?, post_day = ?, post_month = ?, ruud_money = ?, level_majestic = ?, experience_majestic = ?, points_majestic = ?, hunting_log_visible = ?, reset = ?, monster_soul_purchase = ? "
