...
void ArkaWar::LoadIni() {
	if (boost::filesystem::exists(ARKAWAR_FILEPATH)) {
		char arkawarserver[10];

		GetPrivateProfileString(ARKAWAR_SECTION, "Server", "", arkawarserver, sizeof(arkawarserver), ARKAWAR_FILEPATH);
		//sGameServer->SetDevilSquareEnabled(GetPrivateProfileInt(ARKAWAR_SECTION, "Enabled", 0, ARKAWAR_FILEPATH));
		sGameServer->SetArkaWarEnabled(IsServerIN(arkawarserver));
		sGameServer->SetArkaWarRegisterMinMembers(GetPrivateProfileInt(ARKAWAR_SECTION, "RegisterMinMembers", 0, ARKAWAR_FILEPATH));
		sGameServer->SetArkaWarRegisterMaxMembers(GetPrivateProfileInt(ARKAWAR_SECTION, "RegisterMaxMembers", 0, ARKAWAR_FILEPATH));
		sGameServer->SetArkaWarMinGuilds(GetPrivateProfileInt(ARKAWAR_SECTION, "MinGuilds", 0, ARKAWAR_FILEPATH));
		sGameServer->SetArkaWarDamageReductionAlly(GetPrivateProfileInt(ARKAWAR_SECTION, "DamageReductionAlly", 0, ARKAWAR_FILEPATH));
		sGameServer->SetArkaWarDamageReductionEnemy(GetPrivateProfileInt(ARKAWAR_SECTION, "DamageReductionEnemy", 0, ARKAWAR_FILEPATH));

	}
	else {
		sLog->outError(LOG_DEFAULT, "Error Load ArkaWar.ini File");
		MessageBox(NULL, "Error Load ArkaWar.ini File", "Error", ERROR);
		exit(0);
	}
}
...
void ArkaWar::Load()
{
	...
	ReadScript file("../Data/Event/EventData/ArkaWar.txt", "rb");
...
void ArkaWar::CheckObeliskAuraPlayer()
{
	...
	///- Reviso si alg�n jugador est� sobre un aura del obelisk
	PlayerSessionMap const& characters = sObjectMgr->GetAllCharacters();
	for ( PlayerSessionMap::const_iterator it = characters.begin(); it != characters.end(); ++it )
	{
		Player* pPlayer = it->second;
