...
void BloodCastle::ProcState_Playing()
{
	if ( this->SecondPassed() )
	{
		...
		if ( this->GetSubState() == BLOOD_CASTLE_SUBSTATE_WEAPON_DELIVERED )
		{
			...
			Player* pPlayerDelivered = this->GetWeaponOwner();

			pPlayerDelivered->ItemDelete(ITEMGET(13, 19), this->GetWeaponLevel(), 1);
		}
	}
...
void BloodCastle::UpdatePlayerStatus()
{
	for ( uint32 i = 0; i < this->GetPlayerSize(); ++i )
	{
		...
		if ( !pPlayer->IsLive() )
		{
			...
			//pPlayer->SendNotice(NOTICE_NORMAL_BLUE, "Blood Castle initialization failed");
...
void BloodCastle::SendStandByStart()
{
	...
	for ( uint32 i = 0; i < this->GetPlayerSize(); ++i )
	{
		...
		//pPlayer->SendNotice(NOTICE_NORMAL_BLUE, "Blood Castle %d will start after %d seconds ", this->GetGround() + 1, remain);
...
void BloodCastle::SendStart()
{
	for ( uint32 i = 0; i < this->GetPlayerSize(); ++i )
	{
		//PLAYER_POINTER(this->GetPlayer(i)->GetPlayer())->SendNotice(NOTICE_GLOBAL, "Blood Castle %d started", this->GetGround()+1);
...
void BloodCastle::SendMonsterTerminated()
{
	for ( uint32 i = 0; i < this->GetPlayerSize(); ++i )
	{
		//PLAYER_POINTER(this->GetPlayer(i)->GetPlayer())->SendNotice(NOTICE_GLOBAL, "The Blood Castle monster has been eliminated ");
...
void BloodCastle::SendBossTerminated()
{
	for ( uint32 i = 0; i < this->GetPlayerSize(); ++i )
	{
		//PLAYER_POINTER(this->GetPlayer(i)->GetPlayer())->SendNotice(NOTICE_GLOBAL, "The Blood Castle Boss has been eliminated ");
...
void BloodCastle::ProcessDoorDestroyed(Player* pKiller, Monster* pGate)
{
	...
	!pKiller ? sObjectMgr->SendNoticeToMap(this->GetWorldID(), NOTICE_GLOBAL, sMessage->Get(MSG_BLOOD_CASTLE_GATE_DESTROYED_1).c_str()):
		sObjectMgr->SendNoticeToMap(this->GetWorldID(), NOTICE_GLOBAL, sMessage->Get(MSG_BLOOD_CASTLE_GATE_DESTROYED_2).c_str(), pKiller->GetName());
...
void BloodCastle::ProcessStatueDestroyed(Player* pKiller, Monster* pStatue)
{
	...
	this->SetKillerStatue(pKiller);

	!pKiller ? sObjectMgr->SendNoticeToMap(this->GetWorldID(), NOTICE_GLOBAL, sMessage->Get(MSG_BLOOD_CASTLE_STATUE_DESTROYED_1).c_str()):
		sObjectMgr->SendNoticeToMap(this->GetWorldID(), NOTICE_GLOBAL, sMessage->Get(MSG_BLOOD_CASTLE_STATUE_DESTROYED_2).c_str(), pKiller->GetName());

	sObjectMgr->SendNoticeToMap(this->GetWorldID(), NOTICE_GLOBAL, "Blood Castle Statue has been destroyed");
...
void BloodCastle::SendReward()
{
	...
	for ( uint32 i = 0; i < this->GetPlayerSize(); ++i )
	{
		...
		if (this->CheckPlayerComplete(pPlayer)) {
			sItemMgr->ItemSerialCreateGremoryCase(pPlayer, Item(JEWEL::CHAOS, 0, 1, 0, 0, 0, 0, 0, nullptr, 0xFF, 604800), GREMORY_CASE_TYPE_CHARACTER, GremoryCaseReward::BLOOD_CASTLE);
			sEventMgr->GiveContributionReward(pPlayer, EVENT_BLOOD_CASTLE, this->GetGround() + 1, 1, GremoryCaseReward::BLOOD_CASTLE);
		}
		else {
			Party* party = pPlayer->GetParty();
			bool sendRewardParty = false;

			if (party) {
				for (uint8 j = 0; j < party->GetMemberCount(); j++) {
					if (!party->GetMember(j)->GetPlayer())
						continue;

					Player* pPlayerParty = party->GetMember(j)->GetPlayer();

					if (this->CheckPlayerComplete(pPlayerParty)) {
						sEventMgr->GiveContributionReward(pPlayer, EVENT_BLOOD_CASTLE, this->GetGround() + 1, 1, GremoryCaseReward::BLOOD_CASTLE);
						sendRewardParty = true;
					}
				}
			}
...
{
	this->GetWeaponOwner() ? 
		sObjectMgr->SendNoticeToMap(this->GetWorldID(), NOTICE_GLOBAL, sMessage->Get(MSG_BLOOD_CASTLE_COMPLETED_1).c_str(), this->GetWeaponOwner()->GetName()):
		sObjectMgr->SendNoticeToMap(this->GetWorldID(), NOTICE_GLOBAL, sMessage->Get(MSG_BLOOD_CASTLE_COMPLETED_2).c_str());}
...
bool BloodCastle::CheckPicker(Player* pPlayer) {
	if (!pPlayer)
		return false;
	...
	if (party) {
		for (uint8 i = 0; i < party->GetMemberCount(); i++) {
			if (party->GetMember(i)->GetPlayer() == this->GetKillerStatue()) {
				return true;
			}
		}
	}

	if (pPlayer != this->GetKillerStatue())
		return false;
...
void BloodCastle::SetWeaponStatus(Player* pPlayer)
{
	if ( this->GetSubState() != BLOOD_CASTLE_SUBSTATE_WEAPON_DELIVERED && this->GetSubState() != BLOOD_CASTLE_SUBSTATE_COMPLETE )
	{
		if ( !pPlayer )
		{
			...
			for ( uint32 i = 0; i < this->GetPlayerSize(); ++i )
			{
				//PLAYER_POINTER(this->GetPlayer(i)->GetPlayer())->SendNotice(NOTICE_GLOBAL, "Blood Castle dropped Archengel Weapon");
		...
		else
		{
			...
			*/
			...
			for ( uint32 i = 0; i < this->GetPlayerSize(); ++i )
			{
				...
				case 0:
					{
						//pPlayerBC->SendNotice(NOTICE_GLOBAL, "%s obtained Absolute Staff of the Archangel", this->GetWeaponOwnerName());
				...
				case 1:
					{
						...
						//pPlayerBC->SendNotice(NOTICE_GLOBAL, "%s obtained Absolute Sword of the Archangel", this->GetWeaponOwnerName());
					} break;

				case 2:
					{
						pPlayerBC->SendNotice(NOTICE_GLOBAL, sMessage->Get(MSG_BLOOD_CASTLE_ARCHANGEL_WEAPON_3).c_str(), this->GetWeaponOwnerName());
						//pPlayerBC->SendNotice(NOTICE_GLOBAL, "%s obtained Absolute Crossbow of the Archangel", this->GetWeaponOwnerName());
					} break;
				}
			}
		}
...
void CBloodCastleMgr::LoadBloodCastIni() {
	if (boost::filesystem::exists(BLOOD_CASTLE_FILEPATH)) {
		sGameServer->SetBloodCastleEnabled(GetPrivateProfileInt(BLOOD_CASTLE_SECTION, "Enabled", 0, BLOOD_CASTLE_FILEPATH));
		sGameServer->SetBloodCastleDeliverWeaponTime(GetPrivateProfileInt(BLOOD_CASTLE_SECTION, "DeliverWeaponTime", 0, BLOOD_CASTLE_FILEPATH) * IN_MILLISECONDS);
		sGameServer->SetBloodCastleSkillRushCheck(GetPrivateProfileInt(BLOOD_CASTLE_SECTION, "RewardScore", 0, BLOOD_CASTLE_FILEPATH));
		sGameServer->SetBloodCastleRewardScore(GetPrivateProfileInt(BLOOD_CASTLE_SECTION, "SkillRushCheck", 0, BLOOD_CASTLE_FILEPATH));
	}
	else {
		sLog->outError(LOG_DEFAULT, "Error Load BloodCastle.ini File");
		MessageBox(NULL, "Error Load BloodCastle.ini File", "Error", ERROR);
		exit(0);
	}
}
...
void CBloodCastleMgr::LoadBloodCastleSetting()
{
	...
	ReadScript file("../Data/Event/EventData/BloodCastle.txt", "rb");
