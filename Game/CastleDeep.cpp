...
void CastleDeep::LoadIni() {
	if (boost::filesystem::exists(CASTLEDEEP_FILEPATH)) {
		sGameServer->SetCastleDeepEnabled(GetPrivateProfileInt(CASTLEDEEP_SECTION, "Enabled", 0, CASTLEDEEP_FILEPATH));
	}
	else {
		sLog->outError(LOG_DEFAULT, "Error Load CastleDeep.ini File");
		MessageBox(NULL, "Error Load CastleDeep.ini File", "Error", ERROR);
		exit(0);
	}
...
void CastleDeep::Load()
{
	...
	ReadScript file("../Data/Event/EventData/CastleDeep.txt", "rb");
