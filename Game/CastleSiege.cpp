...
void CCastleSiege::LoadIni() {
	if (boost::filesystem::exists(CASTLE_SIEGE_FILEPATH)) {
		uint8 enabledserver = GetPrivateProfileInt(CASTLE_SIEGE_SECTION, "Enabled", 999, CASTLE_SIEGE_FILEPATH);
		if (sGameServer->GetServerCode() == enabledserver) {
			sGameServer->SetCastleSiegeEnabled(true);
		}

		sGameServer->SetCastleSiegeCrownTime(GetPrivateProfileInt(CASTLE_SIEGE_SECTION, "CrownTime", 0, CASTLE_SIEGE_FILEPATH));
		sGameServer->SetCastleSiegeLordMixMax(GetPrivateProfileInt(CASTLE_SIEGE_SECTION, "LordMixMax", 0, CASTLE_SIEGE_FILEPATH));
		sGameServer->SetCastleSiegeMachineEnabled(GetPrivateProfileInt(CASTLE_SIEGE_SECTION, "MachineEnabled", 0, CASTLE_SIEGE_FILEPATH));
		sGameServer->SetCastleSiegeParticipantRewardMinutes(GetPrivateProfileInt(CASTLE_SIEGE_SECTION, "ParticipantRewardMinutes", 0, CASTLE_SIEGE_FILEPATH) * IN_MILLISECONDS);
		sGameServer->SetCastleSiegeReduceDamageDifSide(GetPrivateProfileInt(CASTLE_SIEGE_SECTION, "ReduceDamageDifSide", 0, CASTLE_SIEGE_FILEPATH));
		sGameServer->SetCastleSiegeReduceDamageSameSide(GetPrivateProfileInt(CASTLE_SIEGE_SECTION, "ReduceDamageSameSide", 0, CASTLE_SIEGE_FILEPATH));

		sGameServer->SetCastleSiegeRegisterEnabled(GetPrivateProfileInt(CASTLE_SIEGE_SECTION_REGISTER, "RegisterEnabled", 0, CASTLE_SIEGE_FILEPATH));
		sGameServer->SetCastleSiegeRegisterSignEnabled(GetPrivateProfileInt(CASTLE_SIEGE_SECTION_REGISTER, "RegisterSignEnabled", 0, CASTLE_SIEGE_FILEPATH));
		sGameServer->SetCastleSiegeRegisterMinLevel(GetPrivateProfileInt(CASTLE_SIEGE_SECTION_REGISTER, "RegisterMinLevel", 0, CASTLE_SIEGE_FILEPATH));
		sGameServer->SetCastleSiegeRegisterMinMembers(GetPrivateProfileInt(CASTLE_SIEGE_SECTION_REGISTER, "RegisterMinMembers", 0, CASTLE_SIEGE_FILEPATH));
	}
	else {
		sLog->outError(LOG_DEFAULT, "Error Load ChaosCastle.ini File");
		MessageBox(NULL, "Error Load ChaosCastle.ini File", "Error", ERROR);
		exit(0);
	}
...
void CCastleSiege::LoadData()
{
	...
	ReadScript file("../Data/Event/EventData/CastleSiege.txt", "rb");
...
void CCastleSiege::LoadNPC(uint8 * Packet)
{
	...
	///- Tengo que borrar los mobs as� los agrego nuevamente
	for ( CastleSiegeNpcDataList::iterator it = this->m_npc_data.begin(); it != this->m_npc_data.end(); ++it )
	{
...
void CCastleSiege::VerifyNPCExist() ///- Esta funci�n es para actualizar los datos de la DB en base al status actual de los monsters
{
	for ( CastleSiegeNpcDataList::iterator it = this->m_npc_data.begin(); it != this->m_npc_data.end(); ++it )
	{
		if ( !(*it)->store_in_db ) ///- Solamente debo dejar como inactivos los NPC que son de Base de Datos
		...
		if ( !pMonster->IsLive() || !pMonster->IsPlaying() ) ///- Si el monster est� muerto o fuera de juego lo dejo inactivo
		{
			(*it)->SetActive(false);
			continue;
		}
...
void CCastleSiege::CloseCastleGate()
{
	HASH_SUMMON(i) ///- Recorro todos los monsters activos
	{
		...
		if ( !pObject ) ///- Reviso que sea un objeto v�lido
			continue;

		Monster* pMonster = pObject->ToCreature();
		
		if ( !pMonster ) ///- Reviso que sea un monster v�lido
			continue;
		
		if ( !pMonster->IsPlaying() ) ///- Reviso que est� activo
			continue;
...
void CCastleSiege::UpdateTime()
{
	...
	if ( this->IsNpcLoaded() ) ///- Solamente hago un guardado peri�dico si me lleg� la lista de NPC
	{
...
bool CCastleSiege::GuardianStatueExist()
{
	HASH_SUMMON(i) ///- Recorro todos los monsters activos
	{
		...
		if ( !pMonster->IsPlaying() || !pMonster->IsLive() ) ///- Reviso que est� activo
			continue;
...
void CCastleSiege::ChangeState(uint8 state, bool command, int32 duration)
{
	...
	{
		///- Tanto por comando como para el state de batalla, hay que ponerle la duraci�n m�xima
...
void CCastleSiege::NpcBuyRequest(Player* pPlayer, uint8 * Packet)
{
	...
	if ( pMonster && pMonster->IsLive() && pMonster->IsPlaying() ) ///- Si el monster est� activo, entonces no deber�a poder comprarlo
	{
...
void CCastleSiege::NpcRepairRequest(Player* pPlayer, uint8 * Packet)
{
	...
	if ( !pMonster || !pMonster->IsPlaying() || !pMonster->IsLive() ) ///- Si el monster no est� activo, entonces no puedo repararlo
	{
		this->NpcRepairResult(pPlayer, 0, lpMsg->npc, lpMsg->id, 0, 0);
		return;
	}
