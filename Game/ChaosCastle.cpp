...
void ChaosCastle::SendStart()
{
	if ( this->GetEvent() == EVENT_CHAOS_CASTLE_SURVIVAL )
	{
		...
		sObjectMgr->SendNoticeToMap(this->GetWorldID(), NOTICE_GLOBAL, "Chaos Castle Survival of the Fittest has started");
	}
	else
	{
		//this->SendNotice(NOTICE_GLOBAL, "Chaos Castle %d has already commenced!", this->GetGround() + 1);
		sObjectMgr->SendNoticeToMap(this->GetWorldID(), NOTICE_GLOBAL, "Chaos Castle %d has already commenced!", this->GetGround() + 1);
	}
}
...
void CChaosCastleMgr::LoadIni() {
	if (boost::filesystem::exists(CHAOS_CASTLE_FILEPATH)) {
		sGameServer->SetChaosCastleEnabled(GetPrivateProfileInt(CHAOS_CASTLE_SECTION, "Enabled", 0, CHAOS_CASTLE_FILEPATH));

		sGameServer->SetChaosCastleSurvivalOfTheFittestEnabled(GetPrivateProfileInt(CHAOS_CASTLE_SECTION_SURVIVAL, "SurvivalOfTheFittestEnabled", 0, CHAOS_CASTLE_FILEPATH));
		sGameServer->SetChaosCastleSurvivalMaxPerPC(GetPrivateProfileInt(CHAOS_CASTLE_SECTION_SURVIVAL, "SurvivalMaxPerPC", 0, CHAOS_CASTLE_FILEPATH));
...
void CChaosCastleMgr::LoadChaosCastleSetting()
{
	...
	ReadScript file("../Data/Event/EventData/ChaosCastle.txt", "rb");
