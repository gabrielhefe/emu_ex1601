...
{
	DECLARE_PROPERTY_ARRAY(uint64, Stat, MAX_STAT_TYPE);
	...
	DECLARE_ENUM(uint64, Zen);
	DECLARE_ENUM(uint64, Ruud);

	DECLARE_ENUM(uint8, ExpandedInventory);
};
...
struct CharacterBaseBuff {
	DECLARE_ENUM(uint8, Class);
	DECLARE_ENUM(uint16, BuffID);
	DECLARE_ENUM(uint8, Flags);
	DECLARE_ENUM(uint64, Duration);
};
...
typedef std::vector<CharacterBaseBuff*> CharacterBuff;
...
class CharacterBaseMgr
{
	...
	private:
		...
		CharacterBuff m_character_buff[Character::MAX_CLASS];
	...
	public:
		...
		void LoadCharacterBase(char* pchFileName);
		void LoadCharacterBaseSkill(char* pchFileName);
		void LoadCharacterExperienceNormal(char* pchFileName);
		void LoadCharacterExperienceMaster(char* pchFileName);
		void LoadCharacterExperienceMajestic(char* pchFileName);

		void LoadCharacterHelper(char* pchFileName);
		void LoadCharacterFormula(char* pchFileName);
		void LoadCharacterStatFruit(char* pchFileName);
		void LoadCharacterSpecialization(char* pchFileName);
		void LoadCharacterExperienceAdjust(char* pchFileName);
		void LoadCharacterExperienceBonus(char* pchFileName);
		void LoadCharacterBonus(char* pchFileName);
		void LoadCharacterMaxLevelReward(char* pchFileName);
		void LoadCharacterExperienceTable(char* pchFileName);

		void LoadCalcCharacterPvPDame();
		int CalcCharacterPVP[Character::MAX_CLASS][Character::MAX_CLASS];
		...
		void GenerateCharacterBuffList(Player* pPlayer);
		void GenerateCharacterMoneyAdd(Player* pPlayer);

		int GetCalcPVPDamage(uint8 pPlayerCaster, uint8 pPlayerTarget, uint64 damage);
