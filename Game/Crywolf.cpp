...
void Crywolf::LoadIni() {
	if (boost::filesystem::exists(CRYWOLF_FILEPATH)) {
		sGameServer->SetCrywolfEnabled(GetPrivateProfileInt(CRYWOLF_SECTION, "Enabled", 0, CRYWOLF_FILEPATH));

		sGameServer->crywolf_aichange_time.set(GetPrivateProfileInt(CRYWOLF_SECTION_AI, "Time", 0, CRYWOLF_FILEPATH) * IN_MILLISECONDS);
		sGameServer->crywolf_balgass_start_time.set(GetPrivateProfileInt(CRYWOLF_SECTION_BALGASS, "StartTime", 0, CRYWOLF_FILEPATH) * IN_MILLISECONDS);

		sGameServer->SetCrywolfBenefitApply(GetPrivateProfileInt(CRYWOLF_SECTION_BENEFIT, "Apply", 0, CRYWOLF_FILEPATH));
		sGameServer->SetCrywolfBenefitKundunRefill(GetPrivateProfileInt(CRYWOLF_SECTION_BENEFIT, "KundunRefill", 0, CRYWOLF_FILEPATH));
		sGameServer->SetCrywolfBenefitMonsterHP(GetPrivateProfileInt(CRYWOLF_SECTION_BENEFIT, "MonsterHp", 0, CRYWOLF_FILEPATH));
		sGameServer->SetCrywolfBenefitPlusRate(GetPrivateProfileInt(CRYWOLF_SECTION_BENEFIT, "PlusRate", 0, CRYWOLF_FILEPATH));

		sGameServer->SetCrywolfPenaltyApply(GetPrivateProfileInt(CRYWOLF_SECTION_PENALTY, "Apply", 0, CRYWOLF_FILEPATH));
		sGameServer->SetCrywolfPenaltyJewelDrop(GetPrivateProfileInt(CRYWOLF_SECTION_PENALTY, "JewelDrop", 0, CRYWOLF_FILEPATH));
		sGameServer->SetCrywolfPenaltyExperienceGain(GetPrivateProfileInt(CRYWOLF_SECTION_PENALTY, "ExpGain", 0, CRYWOLF_FILEPATH));

		sGameServer->crywolf_altar_contract_valid_time.set(GetPrivateProfileInt(CRYWOLF_SECTION_ALTAR, "ContractValidationTime", 0, CRYWOLF_FILEPATH) * IN_MILLISECONDS);
		sGameServer->crywolf_altar_contract_last_time.set(GetPrivateProfileInt(CRYWOLF_SECTION_ALTAR, "ContractWaitTime", 0, CRYWOLF_FILEPATH) * IN_MILLISECONDS);
		sGameServer->crywolf_altar_max_contract.set(GetPrivateProfileInt(CRYWOLF_SECTION_ALTAR, "MaxContract", 0, CRYWOLF_FILEPATH));
		sGameServer->crywolf_altar_min_level.set(GetPrivateProfileInt(CRYWOLF_SECTION_ALTAR, "MinLevel", 0, CRYWOLF_FILEPATH));
		sGameServer->SetCrywolfAltarScore(GetPrivateProfileInt(CRYWOLF_SECTION_ALTAR, "Score", 0, CRYWOLF_FILEPATH));
	}
	else {
		sLog->outError(LOG_DEFAULT, "Error Load Crywolf.ini File");
		MessageBox(NULL, "Error Load Crywolf.ini File", "Error", ERROR);
		exit(0);
	}
...
void Crywolf::LoadData()
{
	...
	ReadScript file("../Data/Event/EventData/Crywolf.txt", "rb");
