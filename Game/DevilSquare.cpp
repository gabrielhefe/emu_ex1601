...
void DevilSquareMgr::LoadDevilSquareIni() {
	if (boost::filesystem::exists(DEVIL_SQUARE_FILEPATH)) {
		sGameServer->SetDevilSquareEnabled(GetPrivateProfileInt(DEVIL_SQUARE_SECTION, "Enabled", 0, DEVIL_SQUARE_FILEPATH));
	}
	else {
		sLog->outError(LOG_DEFAULT, "Error Load DevilSquare.ini File");
		MessageBox(NULL, "Error Load DevilSquare.ini File", "Error", ERROR);
		exit(0);
	}
}
...
void DevilSquareMgr::LoadDevilSquareSetting()
{
	...
	ReadScript file("../Data/Event/EventData/DevilSquare.txt", "rb");
...
void DevilSquareMgr::LoadDevilSquareRanking(char* pchFileName)
{
	for ( uint8 i = 0; i < MAX_DEVIL_SQUARE_GROUND; i++ )
	{
		for ( DevilSquareRankingMap::iterator it = this->GetGround(i)->ranking.begin(); it != this->GetGround(i)->ranking.end(); it++ )
	...
	pugi::xml_node Main = file.child("DevilSquareRanking");

	uint32 count = 0;

	for (pugi::xml_node map = Main.child("Ground"); map; map = map.next_sibling()) {
		uint8 ground = map.attribute("Index").as_int();
		if (ground < MAX_DEVIL_SQUARE_GROUND)
		{
			for (pugi::xml_node rank = map.child("Rank"); rank; rank = rank.next_sibling()) {
				devil_square_ranking* add_ranking = new devil_square_ranking;

				uint8 position = rank.attribute("Position").as_int();


				add_ranking->experience.set(rank.attribute("ExpReward").as_int());
				add_ranking->zen.set(rank.attribute("ZenReward").as_int());
