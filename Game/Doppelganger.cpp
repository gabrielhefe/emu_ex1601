...
void Doppelganger::LoadIni() {
	if (boost::filesystem::exists(DOPPELGANGER_FILEPATH)) {
		sGameServer->SetDoppelgangerEnabled(GetPrivateProfileInt(DOPPELGANGER_SECTION, "Enabled", 0, DOPPELGANGER_FILEPATH));
		sGameServer->SetDoppelgangerMonsterSpeed(GetPrivateProfileInt(DOPPELGANGER_SECTION, "MonsterSpeed", 0, DOPPELGANGER_FILEPATH));
		sGameServer->SetDoppelgangerPKCheck(GetPrivateProfileInt(DOPPELGANGER_SECTION, "PKCheck", 0, DOPPELGANGER_FILEPATH));
		sGameServer->SetDoppelgangerPKLevelMax(GetPrivateProfileInt(DOPPELGANGER_SECTION, "PKLevelMax", 0, DOPPELGANGER_FILEPATH));
	}
	else {
		sLog->outError(LOG_DEFAULT, "Error Load Doppelganger.ini File");
		MessageBox(NULL, "Error Load Doppelganger.ini File", "Error", ERROR);
		exit(0);
	}
}

void Doppelganger::LoadLevel(char* pchFileName)
{
	for ( DoppelgangerLevelMap::iterator it = this->level_map.begin(); it != this->level_map.end(); ++it )
	{
		MAP_CLEAR(DoppelgangerMonsterMap::iterator, it->second->monsters);
	...
	pugi::xml_node Main = file.child("DoppelgangerLevel");

	uint32 count = 0;

	for (pugi::xml_node data = Main.child("Data"); data; data = data.next_sibling()) {
		DoppelgangerLevelData* level = new DoppelgangerLevelData;
		level->SetGUID(data.attribute("ID").as_int());
		level->SetLevelMin(data.attribute("LevelMin").as_int());
		level->SetLevelMax(data.attribute("LevelMax").as_int());
		level->SetExperience(data.attribute("Exp").as_int());
		level->SetLarvaRate(data.attribute("LarvaRate").as_int());
		level->SetLarvaCount(data.attribute("LarvaCount").as_int());
		level->SetSilverChestItemBag(data.attribute("SilverChest").as_string());
		level->SetGoldenChestItemBag(data.attribute("GoldenChest").as_string());
...
void Doppelganger::LoadMonster(char* pchFileName)
{
	if ( !sGameServer->IsDoppelgangerEnabled() )
	{
		return;
	...
	pugi::xml_node Main = file.child("DoppelgangerMonsterSetting");

	uint32 count = 0;

	for (pugi::xml_node ground = Main.child("Ground"); ground; ground = ground.next_sibling()) {
		if (DoppelgangerLevelData* pLevel = this->GetLevel(ground.attribute("Index").as_int()))
		{
			for (pugi::xml_node data = ground.child("Data"); data; data = data.next_sibling()) {
				DoppelgangerMonster* monster = new DoppelgangerMonster;
				monster->SetMonster(data.attribute("Monster").as_int());
				monster->SetLevel(data.attribute("Level").as_int());
				monster->SetLife(data.attribute("Life").as_int());
				monster->SetAttackMin(data.attribute("AttackMin").as_int());
				monster->SetAttackMax(data.attribute("AttackMax").as_int());
				monster->SetAttackRate(data.attribute("AttackRate").as_int());
				monster->SetDefense(data.attribute("Def").as_int());
				monster->SetDefenseRate(data.attribute("DefRate").as_int());
...
void Doppelganger::EnterRequest(Player* pPlayer, uint8 * Packet)
{
	...
	if ( !already_exist )
	{
		for ( int32 i = 0; i < MAX_PARTY_MEMBERS; ++i )
		{
			...
			{
				if (pMember->GetWorldId() != WORLD_SILENT) {
					this->EnterResult(pPlayer, 7);
					return;
				}
			}*/

			if (!IN_RANGE(pPlayer, pMember, 15)) {
				this->EnterResult(pPlayer, 7);
				return;
			}
