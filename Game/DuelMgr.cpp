...
void DuelRoom::FinishedOutOfTime()
{
	...
	this->GetDueler(0)->GetPlayer()->GetDuelInfoData(DUEL_INFO)->SetPlayerOutOfTime(true);
	this->GetDueler(1)->GetPlayer()->GetDuelInfoData(DUEL_INFO)->SetPlayerOutOfTime(true);


	if (sGameServer->IsDuelBetItem()) {
		if (this->GetDueler(0)->GetPlayer()->GetDuelInfoData(DUEL_INFO)->GetPlayerOutOfTime() == true) {
			sDuelBetItem->RewardItem(this->GetDueler(0)->GetPlayer(), this->GetDueler(1)->GetPlayer(), 1);
		}

		if (this->GetDueler(1)->GetPlayer()->GetDuelInfoData(DUEL_INFO)->GetPlayerOutOfTime() == true) {
			sDuelBetItem->RewardItem(this->GetDueler(1)->GetPlayer(), this->GetDueler(0)->GetPlayer(), 1);
		}

		this->GetDueler(0)->GetPlayer()->GetBetItemData(BET_ITEM_MAX)->Reset();
		this->GetDueler(1)->GetPlayer()->GetBetItemData(BET_ITEM_MAX)->Reset();
		this->GetDueler(0)->GetPlayer()->GetDuelInfoData(DUEL_INFO)->Reset();
		this->GetDueler(1)->GetPlayer()->GetDuelInfoData(DUEL_INFO)->Reset();
	}
}
...
void DuelRoom::FinishedWinner(Player* winner, Player* looser)
{
	...
	if (sGameServer->IsDuelBetItem()) {
		if (this->GetDueler(0)->GetScore() == DUELING_SCORE) {
			sDuelBetItem->RewardItem(this->GetDueler(0)->GetPlayer(), this->GetDueler(1)->GetPlayer(), 0);
			player_win_name = this->GetDueler(0)->GetPlayer()->GetName();
		}
		else if(this->GetDueler(1)->GetScore() == DUELING_SCORE) {
			sDuelBetItem->RewardItem(this->GetDueler(1)->GetPlayer(), this->GetDueler(0)->GetPlayer(), 0);
			player_win_name = this->GetDueler(1)->GetPlayer()->GetName();
		}

		uint16 char_id_1 = this->GetDueler(0)->GetPlayer()->GetGUID();
		uint16 char_id_2 = this->GetDueler(1)->GetPlayer()->GetGUID();
...
void DuelRoom::FinishedLeftRoom(Player* desertor, bool cancelled)
{
	...
	if ( !cancelled )
	{
		Player* pPlayerLeftRoom = sObjectMgr->FindPlayerByName(NAME_SAFE(desertor));
		if (!pPlayerLeftRoom) {

			Player* check_player = sObjectMgr->FindPlayerByGUID(char_id_1);
			if (check_player) {
				player_win_name = this->GetDueler(0)->GetPlayer()->GetName();
				sDuelBetItem->RewardItem(this->GetDueler(0)->GetPlayer(), this->GetDueler(1)->GetPlayer(), 0);
				this->GetDueler(0)->GetPlayer()->GetBetItemData(BET_ITEM_MAX)->Reset();
			...
			sObjectMgr->SendEventNotification(NOTICE_GLOBAL, "Duel on Room: %d finished. %s win",
				this->GetRoomID() + 1, player_win_name.c_str());
			return;
		}
		
		pPlayerLeftRoom->GetDuelInfoData(DUEL_INFO)->SetPlayerLeftRoom(true);
		...
		if (this->GetDueler(0)->GetPlayer()->GetDuelInfoData(DUEL_INFO)->GetPlayerLeftRoom() == true) {
			this->GetDueler(1)->GetPlayer()->SendNotice(CUSTOM_MESSAGE_ID_RED, "%s has left the room", this->GetDueler(0)->GetPlayer()->GetName());
			sDuelBetItem->RewardItem(this->GetDueler(1)->GetPlayer(), this->GetDueler(0)->GetPlayer(), 0);
			player_win_name = this->GetDueler(1)->GetPlayer()->GetName();

		}
		else if (this->GetDueler(1)->GetPlayer()->GetDuelInfoData(DUEL_INFO)->GetPlayerLeftRoom() == true) {
			this->GetDueler(0)->GetPlayer()->SendNotice(CUSTOM_MESSAGE_ID_RED, "%s has left the room", this->GetDueler(1)->GetPlayer()->GetName());
			sDuelBetItem->RewardItem(this->GetDueler(0)->GetPlayer(), this->GetDueler(1)->GetPlayer(), 0);
			player_win_name = this->GetDueler(0)->GetPlayer()->GetName();
		}
