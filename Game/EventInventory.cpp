...
void EventInventory::LoadIni() {
	if (boost::filesystem::exists(EVENTINVENTORY_FILEPATH)) {
		sGameServer->SetEventInventoryEnabled(GetPrivateProfileInt(EVENTINVENTORY_SECTION, "Enabled", 0, EVENTINVENTORY_FILEPATH));

		sGameServer->SetJewelBingoEnabled(GetPrivateProfileInt(EVENTINVENTORY_SECTION_JEWELBINGO, "Enabled", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetJewelBingoRequiredTicket(GetPrivateProfileInt(EVENTINVENTORY_SECTION_JEWELBINGO, "RequiredTicket", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetJewelBingoReward(GetPrivateProfileInt(EVENTINVENTORY_SECTION_JEWELBINGO, "Reward", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetJewelBingoScoreD(GetPrivateProfileInt(EVENTINVENTORY_SECTION_JEWELBINGO, "ScoreD", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetJewelBingoScoreH(GetPrivateProfileInt(EVENTINVENTORY_SECTION_JEWELBINGO, "ScoreH", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetJewelBingoScoreV(GetPrivateProfileInt(EVENTINVENTORY_SECTION_JEWELBINGO, "ScoreV", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetJewelBingoScoreNotMatched(GetPrivateProfileInt(EVENTINVENTORY_SECTION_JEWELBINGO, "ScoreNotMatched", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetJewelBingoScoreSpecialH(GetPrivateProfileInt(EVENTINVENTORY_SECTION_JEWELBINGO, "ScoreSpecialH", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetJewelBingoScoreSpecialV(GetPrivateProfileInt(EVENTINVENTORY_SECTION_JEWELBINGO, "ScoreSpecialV", 0, EVENTINVENTORY_FILEPATH));

		sGameServer->SetMiniBombEnabled(GetPrivateProfileInt(EVENTINVENTORY_SECTION_MINIBOMB, "Enabled", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetMiniBombRequiredTicket(GetPrivateProfileInt(EVENTINVENTORY_SECTION_MINIBOMB, "RequiredTicket", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetMiniBombReward(GetPrivateProfileInt(EVENTINVENTORY_SECTION_MINIBOMB, "Reward", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetMiniBombRevealScore(GetPrivateProfileInt(EVENTINVENTORY_SECTION_MINIBOMB, "RevealScore", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetMiniBombClearScore(GetPrivateProfileInt(EVENTINVENTORY_SECTION_MINIBOMB, "ClearScore", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetMiniBombFailScore(GetPrivateProfileInt(EVENTINVENTORY_SECTION_MINIBOMB, "FailScore", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetMiniBombFoundedBombsScore(GetPrivateProfileInt(EVENTINVENTORY_SECTION_MINIBOMB, "FoundedBombsScore", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetMiniBombWrongBombsScore(GetPrivateProfileInt(EVENTINVENTORY_SECTION_MINIBOMB, "WrongBombsScore", 0, EVENTINVENTORY_FILEPATH));

		sGameServer->SetMuRoomyEnabled(GetPrivateProfileInt(EVENTINVENTORY_SECTION_MUROOMY, "Enabled", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetMuRoomySpecialEnabled(GetPrivateProfileInt(EVENTINVENTORY_SECTION_MUROOMY, "SpecialEnabled", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetMuRoomyTicketRequired(GetPrivateProfileInt(EVENTINVENTORY_SECTION_MUROOMY, "TicketRequired", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetMuRoomyRewardMoneyAmmount(GetPrivateProfileInt(EVENTINVENTORY_SECTION_MUROOMY, "RewardMoneyAmount", 0, EVENTINVENTORY_FILEPATH));


		sGameServer->SetNumericBaseballEnabled(GetPrivateProfileInt(EVENTINVENTORY_SECTION_BASEBALL, "Enabled", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetNumericBaseballRequiredTicket(GetPrivateProfileInt(EVENTINVENTORY_SECTION_BASEBALL, "RequiredTicket", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetNumericBaseballReward(GetPrivateProfileInt(EVENTINVENTORY_SECTION_BASEBALL, "Reward", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetNumericBaseballScore(0 ,GetPrivateProfileInt(EVENTINVENTORY_SECTION_BASEBALL, "Score1", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetNumericBaseballScore(1, GetPrivateProfileInt(EVENTINVENTORY_SECTION_BASEBALL, "Score2", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetNumericBaseballScore(2, GetPrivateProfileInt(EVENTINVENTORY_SECTION_BASEBALL, "Score3", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetNumericBaseballScore(3, GetPrivateProfileInt(EVENTINVENTORY_SECTION_BASEBALL, "Score4", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetNumericBaseballScore(4, GetPrivateProfileInt(EVENTINVENTORY_SECTION_BASEBALL, "Score5", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetNumericBaseballScoreBall(GetPrivateProfileInt(EVENTINVENTORY_SECTION_BASEBALL, "ScoreBall", 0, EVENTINVENTORY_FILEPATH));
		sGameServer->SetNumericBaseballScoreStrike(GetPrivateProfileInt(EVENTINVENTORY_SECTION_BASEBALL, "ScoreStrike", 0, EVENTINVENTORY_FILEPATH));

	}
	else {
		sLog->outError(LOG_DEFAULT, "Error Load Eveninventory.ini File");
		MessageBox(NULL, "Error Load Eveninventory.ini File", "Error", ERROR);
		exit(0);
	}
}
