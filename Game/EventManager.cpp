...
void CEventMgr::LoadEventData(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EventManager");
	pugi::xml_node invasion = Main.child("Invasion");

	uint32 count = 0;

	uint16 servercode = sGameServer->GetServerCode();

	for (pugi::xml_node data = invasion.child("Data"); data; data = data.next_sibling()) {
		if (sGameServer->IsGSMultiSubEnabled()) {
			if (sGameServer->GetServerType() != 1 || sGameServer->GetServerType() != 2) {
				event_data* add_event = new event_data;

				add_event->SetEventID(8);
				add_event->SetInvasion(data.attribute("EventID").as_int());
				add_event->SetDuration(data.attribute("Duration").as_int());
				add_event->SetNotifyTime(0);
				add_event->SetHour(data.attribute("Hour").as_int());
				add_event->SetMinute(data.attribute("Minute").as_int());
				add_event->SetDayOfWeek(data.attribute("DayOfWeek").as_int());
				add_event->SetDayOfMonth(data.attribute("DayOfMonth").as_int());
				add_event->SetSeasonEvent(-1);
	...
	for (pugi::xml_node events = Main.child("Event"); events; events = events.next_sibling()) {
		/*if (sGameServer->IsGSMultiSubEnabled()) {
			if (servercode != events.attribute("ServerCode").as_int())
				continue;
		}*/

		uint8 eventservercode = events.attribute("ServerCode").as_int();

		if (eventservercode == uint8(-1) || servercode == eventservercode) {
			for (pugi::xml_node data = events.child("Data"); data; data = data.next_sibling()) {
				event_data* add_event = new event_data;
				add_event->SetEventID(events.attribute("EventID").as_int());
				add_event->SetInvasion(0);
				add_event->SetDuration(data.attribute("Duration").as_int());
				add_event->SetNotifyTime(data.attribute("NotifyTime").as_int());
				add_event->SetHour(data.attribute("Hour").as_int());
				add_event->SetMinute(data.attribute("Minute").as_int());
				add_event->SetDayOfWeek(data.attribute("DayOfWeek").as_int());
				add_event->SetDayOfMonth(data.attribute("DayOfMonth").as_int());
...
void CEventMgr::LoadEventSeasonData(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EventSeasonManager");
	...
	for (pugi::xml_node events = Main.child("Event"); events; events = events.next_sibling()) {
		...
		pData->SetName(events.attribute("Name").as_string());
		pData->SetName(events.attribute("Name").as_string());

		QueryResult resultstartTime = MuDatabase.PQuery("SELECT UNIX_TIMESTAMP('%s')", events.attribute("StartDate").as_string());
		Field* fieldsstartTime = resultstartTime->Fetch();
		uint64 starttime = fieldsstartTime[0].GetUInt64();
		pData->SetStartDate(time_t(starttime));

		QueryResult resultendTime = MuDatabase.PQuery("SELECT UNIX_TIMESTAMP('%s')", events.attribute("StartEnd").as_string());
		Field* fieldsendTime = resultendTime->Fetch();
		uint64 endtime = fieldsendTime[0].GetUInt64();
		pData->SetEndDate(time_t(endtime));

		pData->SetOcurrence(events.attribute("Ocurrence").as_int());
		pData->SetLength(events.attribute("Duration").as_int());
		pData->SetStartMessage(events.attribute("StartMessage").as_string());
		pData->SetEndMessage(events.attribute("EndMessage").as_string());

		pData->SetStarted(false);

		this->m_season_data_map[events.attribute("ID").as_int()] = pData;
...
void CEventMgr::LoadEventContributionReward(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EventContributionReward");

	uint32 count = 0;

	for (pugi::xml_node eventload = Main.child("Event"); eventload; eventload = eventload.next_sibling()) {
		for (pugi::xml_node ground = eventload.child("Ground"); ground; ground = ground.next_sibling()) {
			for (pugi::xml_node reward = ground.child("Reward"); reward; reward = reward.next_sibling()) {
				event_contribution_reward* pData = new event_contribution_reward;

				pData->SetEventID(eventload.attribute("EventID").as_int());
				pData->SetEventGround(ground.attribute("ID").as_int());
				pData->SetContributionMin(reward.attribute("ContributionMin").as_int());
				pData->SetContributionMax(reward.attribute("ContributionMax").as_int());
				pData->SetRewardBox(reward.attribute("RewardItemID").as_int());
...
void CEventMgr::LoadEventLevel(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EventLevel");

	uint32 count = 0;

	for (pugi::xml_node eventlevel = Main.child("Event"); eventlevel; eventlevel = eventlevel.next_sibling()) {
		event_level* pData = new event_level;
		pData->SetEventID(eventlevel.attribute("ID").as_int());
		pData->SetEventGround(eventlevel.attribute("Ground").as_int());
		pData->SetNormalLevelMin(eventlevel.attribute("LevelMin").as_int());
		pData->SetNormalLevelMax(eventlevel.attribute("LevelMax").as_int());
...
void CEventMgr::LoadEventEnterCount(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EventEnterCount");
	...
	for (pugi::xml_node events = Main.child("Event"); events; events = events.next_sibling()) {
		...
		pData->SetEventID(events.attribute("ID").as_int());
		pData->SetCount(events.attribute("Count").as_int());
