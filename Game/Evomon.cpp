...
void Evomon::LoadIni() {
	if (boost::filesystem::exists(EVOMON_FILEPATH)) {
		sGameServer->SetEvomonEnabled(GetPrivateProfileInt(EVOMON_SECTION, "Enabled", 0, EVOMON_FILEPATH));
		sGameServer->SetEvomonPartyReward(GetPrivateProfileInt(EVOMON_SECTION, "PartyReward", 0, EVOMON_FILEPATH));
		sGameServer->SetEvomonPartySpecialReward(GetPrivateProfileInt(EVOMON_SECTION, "PartySpecialReward", 0, EVOMON_FILEPATH));
		sGameServer->SetEvomonSend(GetPrivateProfileInt(EVOMON_SECTION, "Send", 0, EVOMON_FILEPATH));
	}
	else {
		sLog->outError(LOG_DEFAULT, "Error Load Evomon.ini File");
		MessageBox(NULL, "Error Load Evomon.ini File", "Error", ERROR);
		exit(0);
	}

}

void Evomon::Load(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EvomonList");

	uint32 count = 0;

	for (pugi::xml_node Evomon = Main.child("Evomon"); Evomon; Evomon = Evomon.next_sibling()) {
		EvomonInfo* info = new EvomonInfo;

		info->SetID(Evomon.attribute("ID").as_int());
		info->SetMonster(Evomon.attribute("Monster").as_int());
		info->SetLevel(Evomon.attribute("Level").as_int());
		info->SetLife(Evomon.attribute("Life").as_int());
		info->SetAttackDamageMin(Evomon.attribute("AttackDamageMin").as_int());
		info->SetAttackDamageMax(Evomon.attribute("AttackDamageMax").as_int());
		info->SetAttackSuccessRate(Evomon.attribute("AttackSuccesRate").as_int());
		info->SetDefense(Evomon.attribute("Defense").as_int());
		info->SetDefenseSuccessRate(Evomon.attribute("DefSuccesRate").as_int());
		info->SetNextID(Evomon.attribute("NextID").as_int());
		info->SetNextIDRate(Evomon.attribute("NextIDRate").as_int());
		info->SetSpecialRate(Evomon.attribute("SpecialRate").as_int());
		info->SetSpecialMonster(Evomon.attribute("SpecialMonster").as_int());
...
void Evomon::LoadReward(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EvomonReward");

	uint32 count = 0;

	for (pugi::xml_node Evomon = Main.child("Reward"); Evomon; Evomon = Evomon.next_sibling()) {
		EvomonReward* info = new EvomonReward;

		info->SetScoreMin(Evomon.attribute("ScoreMin").as_int());
		info->SetScoreMax(Evomon.attribute("ScoreMax").as_int());
		info->SetRewardType(Evomon.attribute("RewardItemCat").as_int());
		info->SetRewardIndex(Evomon.attribute("RewardItemIndex").as_int());
		info->SetRewardCount(Evomon.attribute("RewardCount").as_int());
...
void Evomon::GiveReward(Player* pPlayer, int32 score)
{
	...
	pPlayer->SendNotice(CUSTOM_MESSAGE_ID_BLUE, "score: %d", score);
	...
	if ( sGameServer->IsEvomonPartyReward() )
	{
		...
		for ( int32 i = 0; i < count; ++i )
		{
			...
			if ( Player* pMemberPlayer = pMember[i]->ToPlayer() )
			{
				...
				sItemMgr->ItemSerialCreateGremoryCase(pMemberPlayer, Item(ITEMGET(pReward->GetRewardType(), pReward->GetRewardIndex())), GREMORY_CASE_TYPE_CHARACTER, GremoryCaseReward::EVOMON_REWARD);
			}
		}
	}
	else
