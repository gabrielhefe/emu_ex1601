...
constexpr char* GOBLIN_POINT_FILEPATH = "../Data/GoblinPoint.ini";
constexpr char* GOBLIN_POINT_SECTION = "GoblinPoint";
...
public:
	...
	void LoadBaseData(char* pchFileName);
	void LoadFilter(char* pchFileName);
	void LoadNotice(char* pchFileName);
	void LoadGoblinPoint(char* pchFileName);
	void LoadNonPKTime(char* pchFileName);
	...
	void LoadNotification(char* pchFileName);
	void SendNotification();
	void SendNotification(Player* pPlayer);

	void LoadOffsetData(char* pchFileName);
	void SendOffsetData(Player* pPlayer = nullptr);

	void LoadOffsetFPS(char* pchFileName);
	void SendOffsetFPS(Player* pPlayer = nullptr);

	void LoadCheatScanWhiteList(char* pchFileName);
	bool IsCheatScanWhiteList(std::string const& process_name) const;

	void LoadOfflineAttackSetting(char* pchFileName);
	void LoadOfflineAttackWorld(char* pchFileName);
	void LoadOfflineAttackZone(char* pchFileName);
	...
	DECLARE_ENUM(int32, MaxConnectionPerMAC);
	DECLARE_ENUM(int32, MaxConnectionPerDiskSerial);
	...
	DECLARE_BOOL(DynamicExperience);
	...
	DECLARE_PROPERTY_ARRAY(int64, MaxStat, MAX_STAT_TYPE);
	...
	DECLARE_BOOL(ChannelChangeNotUseHighPKEnabled);
	DECLARE_ENUM(uint8, ChannelChangeNotUseHighPKLevel);
	...
	DECLARE_BOOL(CommandClearInventory);
	DECLARE_BOOL(CommandClearGremoryCase);

	DECLARE_BOOL(CommandAddExpanded);
	...
	DECLARE_ENUM(int32, WalkSpeedDistance2);
	DECLARE_ENUM(int32, WalkSpeedCount2);
	...
	DECLARE_ENUM(int8, SkillBerserkerlDecHP);
	DECLARE_ENUM(int8, SkillDarkNessDecHP);
	...
	DECLARE_BOOL(AntiHackSkillBan);
	DECLARE_BOOL(AntiHackSkillKick);

	DECLARE_ENUM(int32, NormalAttackDiffCheck);
	...
	DECLARE_ENUM(uint8, TopRankingReloadTime);

	DECLARE_BOOL(NoticeEnhancementSuccesAllEnable);
	DECLARE_ENUM(uint8, NoticeEnhancementSuccesMinLevel);

	DECLARE_BOOL(NoticeMixSuccesWingEnable);

	DECLARE_PROPERTY_STRING(SecrecKeyLogin);

	DECLARE_BOOL(NoticeSystemEnabled);
	DECLARE_BOOL(PVPNoticeEnabled);
	DECLARE_BOOL(ItemGetNoticeEnabled);
	DECLARE_BOOL(MonsterKillNoticeEnabled);
	DECLARE_BOOL(MonsterSpawnNoticeEnabled);
	DECLARE_BOOL(MossMerchantNotifyEnbaled);

	DECLARE_BOOL(CheckConnectAccountMacEnable);
	DECLARE_BOOL(CheckConnectAccountDiskSerialEnable);

	DECLARE_BOOL(RankServerEnable);

	DECLARE_BOOL(PersonalStoreCreditsPurcharse);

	DECLARE_ENUM(uint64, JewelsOfBlessSellPrice);
	DECLARE_ENUM(uint64, JewelsOfSoulSellPrice);
	DECLARE_ENUM(uint64, JewelsOfLifeSellPrice);
	DECLARE_ENUM(uint64, JewelsOfChaosSellPrice);
	DECLARE_ENUM(uint64, JewelsOfCreationSellPrice);
	DECLARE_ENUM(uint64, JewelsOfGuardianSellPrice);
	DECLARE_ENUM(uint64, JewelsOfHarmonySellPrice);

	DECLARE_ENUM(uint16, SealedMasteryAddLuckRate);
	DECLARE_BOOL(SealedMasteryAddOptionLevelEnabled);
	DECLARE_ENUM(uint16, SealedMasteryAddOptionLevelMax);
	DECLARE_BOOL(SealedMasteryAddExcellentEnabled);
	DECLARE_RANGE(uint16, SealedMasteryExcellentOption);

	DECLARE_ENUM(uint16, RuudWeaponShopLuckRate);
	DECLARE_ENUM(uint16, RuudWeaponShopSkillRate);
	DECLARE_RANGE(uint16, RuudWeaponShopExeOption);

	DECLARE_ENUM(uint16, DamagePlayerAddPercent);
	DECLARE_ENUM(uint16, DamageToSDPersent);

	DECLARE_BOOL(GSMultiSubEnabled);
	DECLARE_BOOL(PartySummon);

	DECLARE_BOOL(PartyKeepExpSolo);

	DECLARE_ENUM(uint16, ServerTestCodeIndex);

	DECLARE_ENUM(uint32, ExpPoolMuderPerLevel);

	DECLARE_ENUM(uint32, WorldBossRuudReward);

	DECLARE_BOOL(SkillAddDamageEnabled);

	DECLARE_RANGE(uint8, UpgradeArchageWeaponOption);

	DECLARE_BOOL(SellNpcRecoveryEnabled);

	DECLARE_BOOL(LoDeEnabled);

	DECLARE_BOOL(ResetSystemEnbale);
		DECLARE_BOOL(ResetKeepPoints);
		DECLARE_BOOL(ResetReward);
		DECLARE_BOOL(ResetMasterLevel);
		DECLARE_BOOL(ResetMajesticLevel);
		DECLARE_ENUM(int32, MaxReset);
		DECLARE_BOOL(ResetMoveChar);
		DECLARE_BOOL(ResetAutoCommand);
		DECLARE_BOOL(ResetRelog);
