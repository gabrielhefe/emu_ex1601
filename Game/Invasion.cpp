...
void InvasionMgr::LoadInvasionData(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EventInvasion");
	...
	for (pugi::xml_node info = Main.child("Info"); info; info = info.next_sibling()) {
		...
		invasion->invasion.set(info.attribute("ID").as_int());
		invasion->SetName(info.attribute("Name").as_string());
		invasion->SetOnNotify(info.attribute("OnNotify").as_string());
		invasion->SetOnStart(info.attribute("OnStart").as_string());
		invasion->SetOnEnd(info.attribute("OnEnd").as_string());
		invasion->SetOnTimeOut(info.attribute("OnTimeOut").as_string());
		invasion->SetOnKillBoss(info.attribute("OnKillBoss").as_string());
...
void InvasionMgr::LoadInvasionGroup(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EventInvasionGroup");

	uint32 count = 0;

	for (pugi::xml_node inv = Main.child("Invasion"); inv; inv = inv.next_sibling()) {
		for (pugi::xml_node data = inv.child("Data"); data; data = data.next_sibling()) {
			InvasionGroupDB* invasion_gp = new InvasionGroupDB;

			uint32 invasion = inv.attribute("Index").as_int();
			uint32 group = data.attribute("Group").as_int();

			invasion_gp->SetID(data.attribute("Group").as_int());
			invasion_gp->SetRate(data.attribute("Rate").as_int());
			invasion_gp->SetFlag(data.attribute("Flags").as_int());
			invasion_gp->SetWorld(data.attribute("WorldID").as_int());
			invasion_gp->SetX1(data.attribute("GroupX1").as_int());
			invasion_gp->SetY1(data.attribute("GroupY1").as_int());
			invasion_gp->SetX2(data.attribute("GroupX2").as_int());
			invasion_gp->SetY2(data.attribute("GroupY2").as_int());
