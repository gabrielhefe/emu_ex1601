...
void InventoryScript::DeleteItem(uint8 slot)
{
	...
	/*if (!this->CheckInterfacePlayer(this->GetPlayer())) {
		this->GetPlayer()->ClearItem(slot);
	}*/
	this->GetItem(slot)->clear(false);
	//this->GetPlayer()->SaveItemDbBinary();
}

bool InventoryScript::CheckInterfacePlayer(Player* pPlayer) {
	if (pPlayer->InterfaceCheck(pPlayer, InterfaceData::ChaosMachine, 0))
		return true;

	if (pPlayer->InterfaceCheck(pPlayer, InterfaceData::Trade, 1))
		return true;
