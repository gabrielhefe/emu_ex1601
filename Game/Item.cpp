...
bool Item::CheckIsArchngelWeapon() const
{
	return this->GetKind3() == ItemKind3::ARCHANGEL_WEAPON || this->GetKind3() == ItemKind3::BLESSED_ARCHANGEL_WEAPON;
}
	...
	*/
	...
	if ( this->GetMaxSocket() > 0 )
	{
		...
		SOCKET_SLOT_LOOP(i)
		{
			...
			{
				/// - Establezco la opci�n sin nivel
...
void Item::CalculatePrice()
{
	...
	if (this->GetKind1() == ItemKind1::JEWEL) {
		if(this->CalculateSellPriceJewels())
			return;
	}
...
bool Item::CalculateSellPriceJewels()
{
	if (this->GetItem() == ITEMGET(14, 15) || this->GetItem() == RUUD)
	{
		return false;
	...
	{
		Gold = sGameServer->GetJewelsOfBlessSellPrice() * this->GetDurability();
		this->SetSellPrice(Gold);
		return true;
	}
	else if (this->GetItem() == JEWEL::SOUL) // Soul
	{
		Gold = sGameServer->GetJewelsOfSoulSellPrice() * this->GetDurability();
		this->SetSellPrice(Gold);
		return true;
	}
	else if (this->GetItem() == JEWEL::CHAOS) // Chaos
	{
		Gold = sGameServer->GetJewelsOfChaosSellPrice() * this->GetDurability();
		this->SetSellPrice(Gold);
		return true;
	}
	else if (this->GetItem() == JEWEL::CREATION) // Creation
	{
		Gold = sGameServer->GetJewelsOfCreationSellPrice() * this->GetDurability();
		this->SetSellPrice(Gold);
		return true;
	}
	else if (this->GetItem() == JEWEL::LIFE) // Life
	{
		Gold = sGameServer->GetJewelsOfLifeSellPrice() * this->GetDurability();
		this->SetSellPrice(Gold);
		return true;
	}
	else if (this->GetItem() == JEWEL::HARMONY) // Life
	{
		Gold = sGameServer->GetJewelsOfHarmonySellPrice() * this->GetDurability();
		this->SetSellPrice(Gold);
		return true;
	}
	else if (this->GetItem() == JEWEL::GUARDIAN) // Life
	{
		Gold = sGameServer->GetJewelsOfGuardianSellPrice() * this->GetDurability();
		this->SetSellPrice(Gold);
		return true;
	}
...
{
	/*Player* pPlayer = sObjectMgr->FindPlayerByGUID(this->GetOwner());

	this->SaveItemDbBinary(pPlayer);
	this->SaveMuunDbBinary(pPlayer);
	this->SaveEventInventoryDbBinary(pPlayer);
	this->SavePersonalStoreDbBinary(pPlayer);
	this->SaveSellNpcInventoryBinary(pPlayer);
	this->SaveMixRecoveryInventoryBinary(pPlayer);
	this->SaveGremoryCaseAccBinary(pPlayer);
	this->SaveGremoryCaseMobileBinary(pPlayer);
	this->SaveGremoryCaseCharaBinary(pPlayer);
	this->SaveGremoryCasePSBinary(pPlayer);*/

	/*this->SaveItemDbBinary(pPlayer);

	if (this->GetBox() == ITEM_BOX_EVENT_INVENTORY) {
		this->SaveMuunDbBinary(pPlayer);
	}
	else if (this->GetBox() == ITEM_BOX_EVENT_INVENTORY) {
		this->SaveEventInventoryDbBinary(pPlayer);
	}
	else if (this->GetBox() == ITEM_BOX_PERSONAL_STORE) {
		this->SavePersonalStoreDbBinary(pPlayer);
	}
	else if (this->GetBox() == ITEM_BOX_SELL_NPC) {
		this->SaveSellNpcInventoryBinary(pPlayer);
	}
	else if (this->GetBox() == ITEM_BOX_MIX_RECOVERY) {
		this->SaveMixRecoveryInventoryBinary(pPlayer);
	}
	else if (this->GetBox() == ITEM_BOX_GREMORY_CASE_ACC || this->GetBox() == ITEM_BOX_GREMORY_CASE_MOBILE) {
		this->SaveGremoryCaseBinary(pPlayer);
	}
	else if (this->GetBox() == ITEM_BOX_GREMORY_CASE_INV) {
		this->SaveGremoryCaseCharaBinary(pPlayer);
	}
	else if (this->GetBox() == ITEM_BOX_GREMORY_CASE_PS) {
		this->SaveGremoryCasePSBinary(pPlayer);
	}*/
	...
	stmt = MuDatabase.GetPreparedStatement(QUERY_ITEM_DATA_DELETE_BY_SERIAL2);
	...
	/*if (this->CheckSerialDB(this->GetSerial())) {
		QueryResult result = MuDatabase.PQuery("UPDATE IGNORE item_data Set id = %d, box = %d, slot = %d, entry = %d, server = %d, serial = %d, serial_cash_shop = %d, item_data.level = %d, "
			"durability = %d, durability_state = %d, skill = %d, luck = %d, item_data.option = %d, excellent = %d, ancient = %d, option_380 = %d, harmony = %d, socket_1 = %d, socket_2 = %d, "
			"socket_3 = %d, socket_4 = %d, socket_5 = %d, socket_bonus = %d, locked = %d, data_1 = %d, data_2 = %d, data_3 = %d, personal_store_price = %d, personal_store_job = %d, "
			"personal_store_jos = %d, expire_date = %d, flags = %d", this->GetOwner(), this->GetBox(), this->GetOwnerSlot(), this->GetItem(), this->GetSerialServer(), this->GetSerial(), this->GetSerialShop(),
			this->GetLevel(), static_cast<uint8>(this->GetDurability()), static_cast<int32>(this->GetDurabilityState()), this->GetSkill(), this->GetLuck(), this->GetOption(), this->GetExe(), this->GetAncient()
			, this->Get380(), this->GetHarmony(), this->GetSocket(0), this->GetSocket(1), this->GetSocket(2), this->GetSocket(3), this->GetSocket(4), this->GetSocketBonus(), this->IsInventoryActive() ? 1 : 0,
			this->raw.data_extra[0], this->raw.data_extra[1], this->raw.data_extra[2], this->GetPersonalStorePrice(), this->GetPersonalStorePriceJoB(), this->GetPersonalStorePriceJoS(), this->GetExpireDate(),
			this->GetDBFlag());
	}
	else {
		this->DeleteItemDB(this->GetSerial());

		QueryResult result = MuDatabase.PQuery("INSERT IGNORE INTO item_data "
			...
			"VALUES (%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d)", this->GetOwner(), this->GetBox(), this->GetOwnerSlot(), this->GetItem(), this->GetSerialServer(), this->GetSerial(), this->GetSerialShop(),
			this->GetLevel(), static_cast<uint8>(this->GetDurability()), static_cast<int32>(this->GetDurabilityState()), this->GetSkill(), this->GetLuck(), this->GetOption(), this->GetExe(), this->GetAncient()
			, this->Get380(), this->GetHarmony(), this->GetSocket(0), this->GetSocket(1), this->GetSocket(2), this->GetSocket(3), this->GetSocket(4), this->GetSocketBonus(), this->IsInventoryActive() ? 1 : 0,
			this->raw.data_extra[0], this->raw.data_extra[1], this->raw.data_extra[2], this->GetPersonalStorePrice(), this->GetPersonalStorePriceJoB(), this->GetPersonalStorePriceJoS(), this->GetExpireDate(),
			this->GetDBFlag());
...
void Item::LoadDBNew(const char* ItemArray, uint32 owner, uint32 slot, ItemBox box)
{
	Tokenizer tokens(ItemArray, ';');

	std::string ItemEntry = tokens[1];
	std::string SerialServer = tokens[2];
	std::string Serial = tokens[3];
	std::string SerialShop = tokens[4];
	std::string Level = tokens[5];
	std::string Durability = tokens[6];
	std::string DurabilityState = tokens[7];
	std::string Skill = tokens[8];
	std::string Luck = tokens[9];
	std::string Option = tokens[10];
	std::string Exe = tokens[11];
	std::string Ancient = tokens[12];
	std::string Item380 = tokens[13];
	std::string Harmony = tokens[14];
	std::string Socket1 = tokens[15];
	std::string Socket2 = tokens[16];
	std::string Socket3 = tokens[17];
	std::string Socket4 = tokens[18];
	std::string Socket5 = tokens[19];
	std::string SocketBonus = tokens[20];
	std::string InventoryActive = tokens[21];
	std::string data_extra1 = tokens[22];
	std::string data_extra2 = tokens[23];
	std::string data_extra3 = tokens[24];
	std::string PersonalStorePrice = tokens[25];
	std::string PersonalStorePriceJoB = tokens[26];
	std::string PersonalStorePriceJoS = tokens[27];
	std::string ExpireDate = tokens[28];
	std::string DBFlag = tokens[29];

	uint16 ItemEntryInt = atoi(ItemEntry.c_str());
	uint16 SerialServerInt = atoi(SerialServer.c_str());
	uint32 SerialInt = atoi(Serial.c_str());
	uint32 SerialShopInt = atoi(SerialShop.c_str());
	uint8 LevelInt = atoi(Level.c_str());
	uint8 DurabilityInt = atoi(Durability.c_str());
	uint32 DurabilityStateInt = atoi(DurabilityState.c_str());
	uint8 SkillInt = atoi(Skill.c_str());
	uint8 LuckInt = atoi(Luck.c_str());
	uint8 OptionInt = atoi(Option.c_str());
	uint8 ExeInt = atoi(Exe.c_str());
	uint8 AncientInt = atoi(Ancient.c_str());
	uint8 Item380Int = atoi(Item380.c_str());
	uint8 HarmonyInt = atoi(Harmony.c_str());
	uint16 Socket1Int = atoi(Socket1.c_str());
	uint16 Socket2Int = atoi(Socket2.c_str());
	uint16 Socket3Int = atoi(Socket3.c_str());
	uint16 Socket4Int = atoi(Socket4.c_str());
	uint16 Socket5Int = atoi(Socket5.c_str());
	uint8 SocketBonusInt = atoi(SocketBonus.c_str());
	uint8 InventoryActiveInt = atoi(InventoryActive.c_str());
	uint32 data_extra1Int = atoi(data_extra1.c_str());
	uint32 data_extra2Int = atoi(data_extra2.c_str());
	uint32 data_extra3Int = atoi(data_extra3.c_str());
	uint32 PersonalStorePriceInt = atoi(PersonalStorePrice.c_str());
	uint16 PersonalStorePriceJoBInt = atoi(PersonalStorePriceJoB.c_str());
	uint16 PersonalStorePriceJoSInt = atoi(PersonalStorePriceJoS.c_str());
	uint64 ExpireDateInt = atoi(ExpireDate.c_str());
	uint32 DBFlagInt = atoi(DBFlag.c_str());

	this->SetItem(ItemEntryInt);
	this->SetSerialServer(SerialServerInt);
	this->SetSerial(SerialInt);
	this->SetSerialShop(SerialShopInt);
	this->SetLevel(LevelInt);
	this->SetDurability(static_cast<float>(DurabilityInt));
	this->SetDurabilityState(static_cast<float>(DurabilityStateInt));
	this->SetSkill(SkillInt);
	this->SetLuck(LuckInt);
	this->SetOption(OptionInt);
	this->SetExe(ExeInt);
	this->SetAncient(AncientInt);
	this->Set380(Item380Int);
	this->SetHarmony(HarmonyInt);

	this->SetSocket(0, Socket1Int);
	this->SetSocket(1, Socket2Int);
	this->SetSocket(2, Socket3Int);
	this->SetSocket(3, Socket4Int);
	this->SetSocket(4, Socket5Int);

	this->SetSocketBonus(SocketBonusInt);
	this->SetInventoryActive(InventoryActiveInt);

	this->raw.data_extra[0] = data_extra1Int;
	this->raw.data_extra[1] = data_extra2Int;
	this->raw.data_extra[2] = data_extra3Int;
	this->SetPersonalStorePrice(PersonalStorePriceInt);
	this->SetPersonalStorePriceJoB(PersonalStorePriceJoBInt);
	this->SetPersonalStorePriceJoS(PersonalStorePriceJoSInt);
	this->SetExpireDate(ExpireDateInt);
	this->SetDBFlag(DBFlagInt);
...
void Item::LoadDB(const char* ItemArray, uint32 owner, uint32 slot, ItemBox box)
{
	Tokenizer tokens(ItemArray, ';');

	std::string ItemEntry = tokens[2];
	std::string SerialServer = tokens[3];
	std::string Serial = tokens[4];
	std::string SerialShop = tokens[5];
	std::string Level = tokens[6];
	std::string Durability = tokens[7];
	std::string DurabilityState = tokens[8];
	std::string Skill = tokens[9];
	std::string Luck = tokens[10];
	std::string Option = tokens[11];
	std::string Exe = tokens[12];
	std::string Ancient = tokens[13];
	std::string Item380 = tokens[14];
	std::string Harmony = tokens[15];
	std::string Socket1 = tokens[16];
	std::string Socket2 = tokens[17];
	std::string Socket3 = tokens[18];
	std::string Socket4 = tokens[19];
	std::string Socket5 = tokens[20];
	std::string SocketBonus = tokens[21];
	std::string InventoryActive = tokens[22];
	std::string data_extra1 = tokens[23];
	std::string data_extra2 = tokens[24];
	std::string data_extra3 = tokens[25];
	std::string PersonalStorePrice = tokens[26];
	std::string PersonalStorePriceJoB = tokens[27];
	std::string PersonalStorePriceJoS = tokens[28];
	std::string ExpireDate = tokens[29];
	std::string DBFlag = tokens[30];
...
void Item::SaveGremoryCaseAccBinary(Player* pPlayer) {
	if (!pPlayer)
		return;
	...
	for (uint32 i = 0; i < GREMORY_CASE_INVENTORY_SIZE;i++) {
		if (!pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->IsItem())
			continue;

		char buffacc[200];

		uint32 Slot = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetOwnerSlot();
		uint32 ItemId = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetItem();
		uint8 SerialServer = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetSerialServer();
		uint32 Serial = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetSerial();
		uint32 SerialShop = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetSerialShop();
		uint8 ItemLevel = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetLevel();
		uint8 ItemDurability = static_cast<uint8>(pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetDurability());
		uint32 ItemDurabilityState = static_cast<int32>(pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetDurabilityState());
		uint8 ItemSkill = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetSkill();
		uint8 ItemLuck = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetLuck();
		uint8 ItemOption = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetOption();
		uint8 ItemExe = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetExe();
		uint8 ItemAncient = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetAncient();
		uint8 Item380 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->Get380();
		uint8 ItemHarmony = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetHarmony();
		uint16 Socket1 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetSocket(0);
		uint16 Socket2 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetSocket(1);
		uint16 Socket3 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetSocket(2);
		uint16 Socket4 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetSocket(3);
		uint16 Socket5 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetSocket(4);
		uint16 SocketBonus = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetSocketBonus();
		uint8 IsInventoryActive = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->IsInventoryActive() ? 1 : 0;
		uint32 dataExtra0 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->raw.data_extra[0];
		uint32 dataExtra1 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->raw.data_extra[1];
		uint32 dataExtra2 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->raw.data_extra[2];
		uint32 PersonalStorePrice = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetPersonalStorePrice();
		uint16 PersonalStoreJoB = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetPersonalStorePriceJoB();
		uint16 PersonalStoreJoS = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetPersonalStorePriceJoS();
		uint64 ExpireDate = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetExpireDate();
		uint32 DBFlag = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_ACCOUNT)->GetItem(i)->GetDBFlag();
	...
	QueryResult result1 = MuDatabase.PQuery("UPDATE account_warehouse SET gremory_case_acc='%s' where account_id=%d", bufferacc, pPlayer->GetAccountData()->GetGUID());
}

void Item::SaveGremoryCaseMobileBinary(Player* pPlayer) {
	if (!pPlayer)
		return;
	...
	for (uint32 i = 0; i < GREMORY_CASE_INVENTORY_SIZE; i++) {
		if (!pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->IsItem())
			continue;

		char buffmobile[200];

		uint32 Slot = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetOwnerSlot();
		uint32 ItemId = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetItem();
		uint8 SerialServer = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetSerialServer();
		uint32 Serial = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetSerial();
		uint32 SerialShop = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetSerialShop();
		uint8 ItemLevel = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetLevel();
		uint8 ItemDurability = static_cast<uint8>(pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetDurability());
		uint32 ItemDurabilityState = static_cast<int32>(pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetDurabilityState());
		uint8 ItemSkill = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetSkill();
		uint8 ItemLuck = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetLuck();
		uint8 ItemOption = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetOption();
		uint8 ItemExe = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetExe();
		uint8 ItemAncient = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetAncient();
		uint8 Item380 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->Get380();
		uint8 ItemHarmony = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetHarmony();
		uint16 Socket1 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetSocket(0);
		uint16 Socket2 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetSocket(1);
		uint16 Socket3 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetSocket(2);
		uint16 Socket4 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetSocket(3);
		uint16 Socket5 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetSocket(4);
		uint16 SocketBonus = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetSocketBonus();
		uint8 IsInventoryActive = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->IsInventoryActive() ? 1 : 0;
		uint32 dataExtra0 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->raw.data_extra[0];
		uint32 dataExtra1 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->raw.data_extra[1];
		uint32 dataExtra2 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->raw.data_extra[2];
		uint32 PersonalStorePrice = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetPersonalStorePrice();
		uint16 PersonalStoreJoB = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetPersonalStorePriceJoB();
		uint16 PersonalStoreJoS = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetPersonalStorePriceJoS();
		uint64 ExpireDate = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetExpireDate();
		uint32 DBFlag = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_MOBILE)->GetItem(i)->GetDBFlag();
	...
	QueryResult result3 = MuDatabase.PQuery("UPDATE character_gremory_case SET gremory_case_mobile='%s' where guid=%d", buffermobile, pPlayer->GetGUID());
}

void Item::SaveGremoryCaseCharaBinary(Player* pPlayer) {
	if (!pPlayer)
		return;
	...
	for (uint32 i = 0; i < GREMORY_CASE_INVENTORY_SIZE; i++) {
		if (!pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->IsItem())
			continue;

		char buffchara[200];
		char expireDate[20];

		uint32 Slot = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetOwnerSlot();
		uint32 ItemId = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetItem();
		uint8 SerialServer = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetSerialServer();
		uint32 Serial = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetSerial();
		uint32 SerialShop = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetSerialShop();
		uint8 ItemLevel = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetLevel();
		uint8 ItemDurability = static_cast<uint8>(pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetDurability());
		uint32 ItemDurabilityState = static_cast<int32>(pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetDurabilityState());
		uint8 ItemSkill = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetSkill();
		uint8 ItemLuck = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetLuck();
		uint8 ItemOption = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetOption();
		uint8 ItemExe = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetExe();
		uint8 ItemAncient = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetAncient();
		uint8 Item380 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->Get380();
		uint8 ItemHarmony = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetHarmony();
		uint16 Socket1 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetSocket(0);
		uint16 Socket2 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetSocket(1);
		uint16 Socket3 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetSocket(2);
		uint16 Socket4 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetSocket(3);
		uint16 Socket5 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetSocket(4);
		uint16 SocketBonus = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetSocketBonus();
		uint8 IsInventoryActive = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->IsInventoryActive() ? 1 : 0;
		uint32 dataExtra0 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->raw.data_extra[0];
		uint32 dataExtra1 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->raw.data_extra[1];
		uint32 dataExtra2 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->raw.data_extra[2];
		uint32 PersonalStorePrice = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetPersonalStorePrice();
		uint16 PersonalStoreJoB = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetPersonalStorePriceJoB();
		uint16 PersonalStoreJoS = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetPersonalStorePriceJoS();
		uint64 ExpireDate = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetExpireDate();
		uint32 DBFlag = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_CHARACTER)->GetItem(i)->GetDBFlag();
	...
	QueryResult result2 = MuDatabase.PQuery("UPDATE character_gremory_case SET gremory_case_char='%s' where guid=%d", bufferchar, pPlayer->GetGUID());
}

void Item::SaveGremoryCasePSBinary(Player* pPlayer) {
	//save gremory case ps
	std::ostringstream itemGremoryCasePS;

	for (uint32 i = 0; i < GREMORY_CASE_PERSONAL_STORE_SIZE; i++) {
		if (!pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->IsItem())
			continue;

		char buffps[200];

		uint32 Slot = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetOwnerSlot();
		uint32 ItemId = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetItem();
		uint8 SerialServer = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetSerialServer();
		uint32 Serial = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetSerial();
		uint32 SerialShop = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetSerialShop();
		uint8 ItemLevel = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetLevel();
		uint8 ItemDurability = static_cast<uint8>(pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetDurability());
		uint32 ItemDurabilityState = static_cast<int32>(pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetDurabilityState());
		uint8 ItemSkill = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetSkill();
		uint8 ItemLuck = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetLuck();
		uint8 ItemOption = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetOption();
		uint8 ItemExe = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetExe();
		uint8 ItemAncient = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetAncient();
		uint8 Item380 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->Get380();
		uint8 ItemHarmony = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetHarmony();
		uint16 Socket1 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetSocket(0);
		uint16 Socket2 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetSocket(1);
		uint16 Socket3 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetSocket(2);
		uint16 Socket4 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetSocket(3);
		uint16 Socket5 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetSocket(4);
		uint16 SocketBonus = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetSocketBonus();
		uint16 IsInventoryActive = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->IsInventoryActive() ? 1 : 0;
		uint32 dataExtra0 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->raw.data_extra[0];
		uint32 dataExtra1 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->raw.data_extra[1];
		uint32 dataExtra2 = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->raw.data_extra[2];
		uint32 PersonalStorePrice = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetPersonalStorePrice();
		uint16 PersonalStoreJoB = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetPersonalStorePriceJoB();
		uint16 PersonalStoreJoS = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetPersonalStorePriceJoS();
		uint64 ExpireDate = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetExpireDate();
		uint32 DBFlag = pPlayer->GetGremoryCase(GREMORY_CASE_TYPE_PERSONAL_STORE)->GetItem(i)->GetDBFlag();
	...
	QueryResult result4 = MuDatabase.PQuery("UPDATE character_gremory_case SET gremory_case_ps='%s' where guid=%d", bufferps, pPlayer->GetGUID());
}