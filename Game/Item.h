...
class Item
{
	public:
		...
		DECLARE_PROPERTY_ARRAY(int64, StatRequired, MAX_STAT_TYPE);
		DECLARE_PROPERTY_ARRAY(int64, StatDecrease, MAX_STAT_TYPE);
	...
	public:
		...
		bool CheckIsArchngelWeapon() const;
		...
		void LoadDBNew(const char* ItemArray, uint32 owner, uint32 slot, ItemBox box);
		void LoadDB(const char* ItemArray, uint32 owner, uint32 slot, ItemBox box);
		...
		void SaveGremoryCaseAccBinary(Player* pPlayer);
		void SaveGremoryCaseMobileBinary(Player* pPlayer);
		void SaveGremoryCaseCharaBinary(Player* pPlayer);
		void SaveGremoryCasePSBinary(Player* pPlayer);
	private:
