...
bool ItemBag::Execute(Unit* pUnit, int32 data_1, int32 data_2, int16 x, int16 y, uint16 serial_server, uint32 serial, std::string const& item_name)
{
	...
	uint8 ItemBagType = this->GetType();
	...
	case ITEM_BAG_EVENT:
		{
			if ( pMonster )
			{
				...
				this->Drop(pMonster->GetLastAttacker(), pMonster->GetWorldId(), pMonster->GetX(), pMonster->GetY(), pMonster->GetElementalAttribute());
			}
		} break;
...
void ItemBag::Drop(Unit* mOwner, uint16 world, int16 x, int16 y, uint8 attribute)
{
	...
	if ( this->GetType() == ITEM_BAG_GROUP_ITEM || this->GetType() == ITEM_BAG_GROUP_MONSTER )
	{
		...
		{
			uint16 randomX1 = 0 - rand() % 3 + 0;
			uint16 randomX2 = rand() % 3 + 0;
			uint16 randomY1 = 0 - rand() % 3 + 0;
			uint16 randomY2 = rand() % 3 + 0;

			if (this->BuildItem(pData, item, mOwner, r_x, r_y, attribute))
			{
				sItemMgr->ItemSerialCreate(mOwner, pWorld->GetEntry(), r_x, r_y, item, true, true, true);
...
void ItemBagMgr::LoadItemBagTemplate(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("ItemBagList");
	...
	for (pugi::xml_node item = Main.child("Item"); item; item = item.next_sibling()) {
		...
		item_bag->SetGUID(item.attribute("ID").as_int());
		item_bag->SetType(item.attribute("Type").as_int());
		item_bag->SetName(item.attribute("Name").as_string());
		item_bag->raw.data[0] = item.attribute("Param1").as_int();
		item_bag->raw.data[1] = item.attribute("Param2").as_int();
		item_bag->raw.data[2] = item.attribute("Param3").as_int();
		item_bag->required_level.set(item.attribute("ReqLevel").as_int());
		item_bag->SetMoneyRate(item.attribute("ZenRate").as_int());
		item_bag->SetMoneyMin(item.attribute("ZenMin").as_int());
		item_bag->SetMoneyMax(item.attribute("ZenMax").as_int());
		item_bag->SetRuudRate(item.attribute("RuudRate").as_int());
		item_bag->SetRuudMin(item.attribute("RuudMin").as_int());
		item_bag->SetRuudMax(item.attribute("RuudMax").as_int());
		item_bag->drop_min.set(item.attribute("DropMin").as_int());
		item_bag->drop_max.set(item.attribute("DropMax").as_int());
		item_bag->SetFlag(item.attribute("FireWorkEffect").as_int());
		item_bag->SetFileScriptName(item.attribute("FileScriptName").as_string());
		...
		std::string pathfile = "../Data/Item/ItemBag/" + item_bag->GetFileScriptName();
		this->LoadItemBagItems(pathfile.c_str(), item_bag->GetGUID());
...
void ItemBagMgr::LoadItemBagItems(const char* pchFileName, uint16 GUID)
{
	pugi::xml_document file;
	pugi::xml_parse_result res = file.load_file(pchFileName);
	...
	pugi::xml_node Main = file.child("ItemBag");

	for (pugi::xml_node bag = Main.child("Item"); bag; bag = bag.next_sibling()) {
		ItemBagItem* item = new ItemBagItem;

		uint32 id = GUID;
		item->SetSort(bag.attribute("Sort").as_int());
		item->SetType(bag.attribute("ItemCat").as_int());
		item->SetIndex(bag.attribute("ItemIndex").as_int());
		...
		item->SetLevelMin(bag.attribute("ItemLevelMin").as_int());
		item->SetLevelMax(bag.attribute("ItemLevelMax").as_int());
		item->SetDurability(bag.attribute("Dur").as_int());
		item->SetSkillRate(bag.attribute("SkillRate").as_int());
		item->SetLuckRate(bag.attribute("LuckRate").as_int());
		item->SetOptionMin(bag.attribute("OptionMin").as_int());
		item->SetOptionMax(bag.attribute("OptionMax").as_int());
		item->SetExeMin(bag.attribute("ExlMin").as_int());
		item->SetExeMax(bag.attribute("ExlMax").as_int());
		item->SetAncient(bag.attribute("Ancient").as_int());
		item->SetAncientOptionMin(bag.attribute("AncientOptionMin").as_int());
		item->SetAncientOptionMax(bag.attribute("AncientOptionMax").as_int());

		item->SetSocketRate(0, bag.attribute("Socket1Rate").as_int());
		item->SetSocketRate(1, bag.attribute("Socket2Rate").as_int());
		item->SetSocketRate(2, bag.attribute("Socket3Rate").as_int());
		item->SetSocketRate(3, bag.attribute("Socket4Rate").as_int());
		item->SetSocketRate(4, bag.attribute("Socket5Rate").as_int());

		item->SetRate(bag.attribute("Rate").as_int());
		item->SetFlag(bag.attribute("FireWork").as_int());
		item->SetGroup(bag.attribute("Group").as_int());
		item->SetDuration(bag.attribute("Duration").as_int());
