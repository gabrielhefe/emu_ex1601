...
void JewelMix::Load(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("MixJewelTemplate");
	...
	for (pugi::xml_node mix = Main.child("Mix"); mix; mix = mix.next_sibling()) {
		for (pugi::xml_node data = mix.child("Data"); data; data = data.next_sibling()) {
			...
			add_jewel->id.set(mix.attribute("ID").as_int());
			add_jewel->type.set(data.attribute("ItemCat").as_int());
			add_jewel->index.set(data.attribute("ItemIndex").as_int());
			add_jewel->level.set(data.attribute("Level").as_int());
			add_jewel->count.set(data.attribute("Count").as_int());
			add_jewel->mix_money.set(data.attribute("MixCost").as_int());
			add_jewel->unmix_money.set(data.attribute("MixCost").as_int());
			add_jewel->pack_type.set(data.attribute("PackCat").as_int());
			add_jewel->pack_index.set(data.attribute("PackIndex").as_int());
			add_jewel->flags.set(3);
