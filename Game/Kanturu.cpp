...
void KanturuMgr::LoadIni() {
	if (boost::filesystem::exists(KANTURU_FILEPATH)) {
		sGameServer->SetKanturuEnabled(GetPrivateProfileInt(KANTURU_SECTION, "Enabled", 0, KANTURU_FILEPATH));
		sGameServer->kanturu_max_players.set(GetPrivateProfileInt(KANTURU_SECTION, "MaxPlayers", 0, KANTURU_FILEPATH));
		sGameServer->kanturu_ice_storm_rate.set(GetPrivateProfileInt(KANTURU_SECTION, "MayaIceStormRate", 0, KANTURU_FILEPATH));
		sGameServer->kanturu_skill_time.set(GetPrivateProfileInt(KANTURU_SECTION, "MayaSkillTime", 0, KANTURU_FILEPATH));
	}
	else {
		sLog->outError(LOG_DEFAULT, "Error Load Kanturu.ini File");
		MessageBox(NULL, "Error Load Kanturu.ini File", "Error", ERROR);
		exit(0);
	}
...
void KanturuMgr::Load()
{
	...
	std::string filename("../Data/Event/EventData/Kanturu.txt");
