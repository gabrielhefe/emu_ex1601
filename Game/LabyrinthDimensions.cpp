...
void LabyrinthDimensions::LoadIni() {
	if (boost::filesystem::exists(LABYRITNTH_FILEPATH)) {
		sGameServer->SetLabyrinthOfDimensionsEnabled(GetPrivateProfileInt(LABYRITNTH_SECTION, "Enabled", 0, LABYRITNTH_FILEPATH));

		sGameServer->SetLabyrinthOfDimensionsRequiredMaster(GetPrivateProfileInt(LABYRITNTH_SECTION, "RequiredMaster", 0, LABYRITNTH_FILEPATH));
		sGameServer->SetLabyrinthOfDimensionsRequiredLevel(GetPrivateProfileInt(LABYRITNTH_SECTION, "RequiredLevel", 0, LABYRITNTH_FILEPATH));
		sGameServer->SetLabyrinthOfDimensionsGoblinScore(GetPrivateProfileInt(LABYRITNTH_SECTION, "GoblinScore", 0, LABYRITNTH_FILEPATH));
		sGameServer->SetLabyrinthOfDimensionsGoblinStage(GetPrivateProfileInt(LABYRITNTH_SECTION, "GoblinStage", 0, LABYRITNTH_FILEPATH));
		sGameServer->SetLabyrinthOfDimensionsMoveOnFail(GetPrivateProfileInt(LABYRITNTH_SECTION, "MoveOnFail", 1, LABYRITNTH_FILEPATH));
		sGameServer->SetLabyrinthOfDimensionsMissionTime(GetPrivateProfileInt(LABYRITNTH_SECTION, "MissionTime", 5, LABYRITNTH_FILEPATH) * IN_MILLISECONDS);
		sGameServer->SetLabyrinthOfDimensionsDayFirstScore(GetPrivateProfileInt(LABYRITNTH_SECTION, "DayFirstScore", 0, LABYRITNTH_FILEPATH));
		sGameServer->SetLabyrinthOfDimensionsStateDuration(GetPrivateProfileInt(LABYRITNTH_SECTION, "StateDuration", 604800, LABYRITNTH_FILEPATH));
		sGameServer->SetLabyrinthOfDimensionsTimeScore(GetPrivateProfileInt(LABYRITNTH_SECTION, "TimeScore", 0, LABYRITNTH_FILEPATH));

		sGameServer->SetLabyrinthOfDimensionsMove(LABYRINTH_OF_DIMENSIONS_RANK_BRONZE, GetPrivateProfileInt(LABYRITNTH_SECTION, "MoveBronze", 0, LABYRITNTH_FILEPATH));
		sGameServer->SetLabyrinthOfDimensionsMove(LABYRINTH_OF_DIMENSIONS_RANK_SILVER, GetPrivateProfileInt(LABYRITNTH_SECTION, "MoveSilver", 0, LABYRITNTH_FILEPATH));
		sGameServer->SetLabyrinthOfDimensionsMove(LABYRINTH_OF_DIMENSIONS_RANK_GOLD, GetPrivateProfileInt(LABYRITNTH_SECTION, "MoveGold", 0, LABYRITNTH_FILEPATH));
		sGameServer->SetLabyrinthOfDimensionsMove(LABYRINTH_OF_DIMENSIONS_RANK_LEGEND, GetPrivateProfileInt(LABYRITNTH_SECTION, "MoveLegend", 0, LABYRITNTH_FILEPATH));
	}
	else {
	sLog->outError(LOG_DEFAULT, "Error Load Labyrinth.ini File");
	MessageBox(NULL, "Error Load Labyrinth.ini File", "Error", ERROR);
	exit(0);
	}
}

void LabyrinthDimensions::LoadInfo(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EventLabyrinthInfo");

	uint32 count = 0;

	for (pugi::xml_node data = Main.child("Data"); data; data = data.next_sibling()) {
		LabyrinthInfo* pData = new LabyrinthInfo;
		pData->SetIndex(data.attribute("Index").as_int());
		pData->SetWorld(data.attribute("World").as_int());
		pData->SetX1(data.attribute("X1").as_int());
		pData->SetY1(data.attribute("Y1").as_int());
		pData->SetX2(data.attribute("X2").as_int());
		pData->SetY2(data.attribute("Y2").as_int());
...
void LabyrinthDimensions::LoadMission(char* pchFileName)
{
	if (!sGameServer->IsLabyrinthOfDimensionsEnabled())
	{
		return;
	...
	pugi::xml_node Main = file.child("EventLabyrinthMission");

	uint32 count = 0;

	for (pugi::xml_node area = Main.child("Area"); area; area = area.next_sibling()) {
		for (pugi::xml_node data = area.child("Data"); data; data = data.next_sibling()) {
			LabyrinthMission* pData = new LabyrinthMission;
			pData->SetAreaType(area.attribute("Type").as_int());
			pData->SetIndex(data.attribute("Index").as_int());

			pData->GetMission(0)->SetType(data.attribute("MissionType1").as_int());
			pData->GetMission(0)->SetValue(data.attribute("MissionValue1").as_int());
			pData->GetMission(1)->SetType(data.attribute("MissionType2").as_int());
			pData->GetMission(1)->SetValue(data.attribute("MissionValue2").as_int());
			pData->GetMission(2)->SetType(data.attribute("MissionType3").as_int());
			pData->GetMission(2)->SetValue(data.attribute("MissionValue3").as_int());
			pData->GetMission(3)->SetType(data.attribute("MissionType4").as_int());
			pData->GetMission(3)->SetValue(data.attribute("MissionValue4").as_int());
			pData->GetMission(4)->SetType(data.attribute("MissionType5").as_int());
			pData->GetMission(4)->SetValue(data.attribute("MissionValue5").as_int());
			pData->GetMission(5)->SetType(data.attribute("MissionType6").as_int());
			pData->GetMission(5)->SetValue(data.attribute("MissionValue6").as_int());

			pData->GetMonster(0)->SetMonster(data.attribute("Monster1").as_int());
			pData->GetMonster(0)->SetCount(data.attribute("Count1").as_int());
			pData->GetMonster(1)->SetMonster(data.attribute("Monster2").as_int());
			pData->GetMonster(1)->SetCount(data.attribute("Count2").as_int());
			pData->GetMonster(2)->SetMonster(data.attribute("Monster3").as_int());
			pData->GetMonster(2)->SetCount(data.attribute("Count3").as_int());
...
void LabyrinthDimensions::LoadArea(char* pchFileName)
{
	if (!sGameServer->IsLabyrinthOfDimensionsEnabled())
	{
		return;
	...
	pugi::xml_node Main = file.child("EventLabyrinthArea");
	...
	for (pugi::xml_node data = Main.child("Data"); data; data = data.next_sibling()) {
		...
		pData->SetIndex(data.attribute("Index").as_int());
		pData->SetStage(data.attribute("Stage").as_int());
		pData->SetRowCount(data.attribute("RowCount").as_int());
		pData->SetColumnCount(data.attribute("ColumnCount").as_int());
...
void LabyrinthDimensions::LoadAreaData(char* pchFileName)
{
	if (!sGameServer->IsLabyrinthOfDimensionsEnabled())
	{
		return;
	...
	pugi::xml_node Main = file.child("EventLabyrinthAreaData");

	uint32 count = 0;

	for (pugi::xml_node area = Main.child("Area"); area; area = area.next_sibling()) {
		for (pugi::xml_node data = area.child("Data"); data; data = data.next_sibling()) {
			LabyrinthAreaMap::iterator it = this->labyrinth_area_map.find(area.attribute("Index").as_int());

			if (it != this->labyrinth_area_map.end())
			{
				LabyrinthAreaData* pData = new LabyrinthAreaData;
				pData->SetSubIndex(data.attribute("Row").as_int());
				pData->SetIndex(data.attribute("Column").as_int());
				pData->SetWorldIndex(data.attribute("WorldIndex").as_int());
				pData->SetAreaType(data.attribute("AreaType").as_int());
				pData->SetMissionID(data.attribute("Index").as_int());

				pData->SetGate(0, data.attribute("Up").as_int());
				pData->SetGate(1, data.attribute("Down").as_int());
				pData->SetGate(2, data.attribute("Left").as_int());
				pData->SetGate(3, data.attribute("Right").as_int());
...
void LabyrinthDimensions::LoadLevel(char* pchFileName)
{
	if (!sGameServer->IsLabyrinthOfDimensionsEnabled())
	{
		return;
	...
	pugi::xml_node Main = file.child("EventLabyrinthLevel");
	...
	for (pugi::xml_node data = Main.child("Data"); data; data = data.next_sibling()) {
		...
		pData->SetFirstScore(data.attribute("ScoreFirst").as_int());
		pData->SetScore(data.attribute("Score").as_int());
...
void LabyrinthDimensions::LoadMonster(char* pchFileName)
{
	if (!sGameServer->IsLabyrinthOfDimensionsEnabled())
	{
		return;
	...
	pugi::xml_node Main = file.child("EventLabyrinthMonster");
	...
	for (pugi::xml_node data = Main.child("Data"); data; data = data.next_sibling()) {
		...
		pData->SetID(data.attribute("ID").as_int());
		pData->SetLevel(data.attribute("Level").as_float());
		pData->SetHP(data.attribute("HP").as_float());
		pData->SetDamage(data.attribute("Damage").as_float());
		pData->SetDefense(data.attribute("Def").as_float());
		pData->SetAttackSuccessRate(data.attribute("AttackSucces").as_float());
		pData->SetDefenseSuccessRate(data.attribute("DefSucces").as_float());
		pData->SetElementalDamage(data.attribute("ElementalDamage").as_float());
		pData->SetElementalDefense(data.attribute("ElementalDef").as_float());
		pData->SetElementalAttackSuccessRate(data.attribute("ElementalAttackSucces").as_float());
		pData->SetElementalDefenseSuccessRate(data.attribute("ElementDefSucces").as_float());
...
void LabyrinthDimensions::LoadReward(char* pchFileName)
{
	if (!sGameServer->IsLabyrinthOfDimensionsEnabled())
	{
		return;
	...
	pugi::xml_node Main = file.child("EventLabyrinthReward");

	uint32 count = 0;

	for (pugi::xml_node cat = Main.child("Category"); cat; cat = cat.next_sibling()) {
		for (pugi::xml_node rank = cat.child("Rank"); rank; rank = rank.next_sibling()) {
			for (pugi::xml_node data = rank.child("Data"); data; data = data.next_sibling()) {
				LabyrinthReward* pData = new LabyrinthReward;
				pData->SetCategory(cat.attribute("Index").as_int());
				pData->SetRank(rank.attribute("Index").as_int());
				pData->SetClass(data.attribute("Class").as_int());
				pData->SetItem(ITEMGET(data.attribute("Type").as_int(), data.attribute("Index").as_int()));
				pData->SetLevel(data.attribute("Level").as_int());
				pData->SetDurability(data.attribute("Dur").as_int());
				pData->SetSkill(data.attribute("Skill").as_int());
				pData->SetLuck(data.attribute("Luck").as_int());
				pData->SetOption(data.attribute("Option").as_int());
				pData->SetExcellent(data.attribute("Exl").as_int());
				pData->SetAncient(data.attribute("Ancient").as_int());
				if (pData->GetClass() == 255) {
					pData->SetDBFlags(0);
				}
				else {
					pData->SetDBFlags(17);
