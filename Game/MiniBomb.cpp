...
void MiniBomb::LoadReward(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EventMiniBombReward");
	...
	for (pugi::xml_node data = Main.child("Data"); data; data = data.next_sibling()) {
		...
		add_item->SetScoreMin(data.attribute("ScoreMin").as_int());
		add_item->SetScoreMax(data.attribute("ScoreMax").as_int());
		add_item->SetItem(data.attribute("ItemID").as_int());
		add_item->SetLevel(data.attribute("Level").as_int());
...
void MiniBomb::SendCell(Player* pPlayer, uint8 type)
{
	...
	*/
	...
	head->h.set(HEACODE_EVENT_INVENTORY, SUBCODE_EVENT_INVENTORY_MINIBOMB_OPEN, sizeof(MINI_BOMB_OPEN_HEAD) + (head->count * sizeof(MINI_BOMB_OPEN_BODY)));
...
void MiniBomb::Reveal(Player* pPlayer, uint8 * Packet)
{
	...
	head->h.set(HEACODE_EVENT_INVENTORY, SUBCODE_EVENT_INVENTORY_MINIBOMB_REVEAL, sizeof(MINI_BOMB_REVEAL_HEAD) + (head->count * sizeof(MINI_BOMB_REVEAL_BODY)));
	pPlayer->sendPacket(buffer, head->h.get_size());
...
void MiniBomb::Finish(Player* pPlayer, bool from_game)
{
	...
	head->h.set(HEACODE_EVENT_INVENTORY, SUBCODE_EVENT_INVENTORY_MINIBOMB_RESULT, sizeof(MINI_BOMB_RESULT_HEAD) + (head->count * sizeof(MINI_BOMB_RESULT_BODY)));
	pPlayer->sendPacket(buffer, head->h.get_size());
}
