...
void MiniMap::Load(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("MiniMap");

	uint32 count = 0;

	for (pugi::xml_node map = Main.child("Option"); map; map = map.next_sibling()) {
		MiniMapInfo* info = new MiniMapInfo;

		info->SetWorld(map.attribute("WorldID").as_int());
		info->SetIndex(map.attribute("Index").as_int());
		info->SetGroup(map.attribute("Group").as_int());
		info->SetType(map.attribute("Type").as_int());
		info->SetX(map.attribute("X").as_int());
		info->SetY(map.attribute("Y").as_int());
		info->SetText(map.attribute("Text").as_string());
...
void MiniMap::GCMiniMapPartyInfoSend(Player* player)
{
	...
	head->Set(HEADCODE_MINIMAP_DATA, SUBCODE_MINIMAP_DATA_PARTY_INFO, sizeof(MINI_MAP_PARTY_HEAD)+(head->count * sizeof(MINI_MAP_PARTY_BODY)));
