...
void MiningSystem::Load(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("Mining");
	...
	for (pugi::xml_node data = Main.child("Data"); data; data = data.next_sibling()) {
		...
		info->SetIndex(data.attribute("Index").as_int());
		info->SetStage(data.attribute("Stage").as_int());
		info->SetValue(data.attribute("Value").as_int());
		info->SetItemRate(data.attribute("ItemRate").as_int());
		info->SetItemIndex(data.attribute("ItemIndex").as_int());
		info->SetItemCount(data.attribute("ItemCount").as_int());
		info->SetBonusItemRate(data.attribute("BonusItemRate").as_int());
		info->SetBonusItemIndex(data.attribute("BonuxItemIndex").as_int());
		info->SetBonusItemCount(data.attribute("BonusItemCount").as_int());
...
void MiningSystem::CGMiningStartRecv(Player* pPlayer, uint8 * Packet)
{
	...
	if ( Random(1000000) < pMiningInfo->GetItemRate() )
	{
