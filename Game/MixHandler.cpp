...
void MixHandler::ExecuteMix(uint8 * packet)
{
	...
	if ( this->GetPlayer()->IsMixCompleted() )
	{
		chaos_box_loop(i)
		{
			...
			/*if (item->IsWingLvl2() || item->IsWingLvl3() || item->IsWingLvl4() || item->IsSpecialWing() || item->IsWingMonster()) {
...
void MixHandler::ChaosMixDevilSquare(uint8 data)
{
	...
	for ( int32 i = 0; i < count; ++i )
	{
		...
		//this->SetSuccess(roll_chance_i(8000, 10000) ? 1 : 0);
...
void MixHandler::ChaosMixWingSecond(uint8 data)
{
	...
	if ( this->roll_success() )
	{
		...
		uint8 luck = roll_chance_i(4000, 10000) ? 1 : 0;
...
void MixHandler::ChaosMixWingFirst(uint8 data)
{
	...
	chaos_box_loop(i)
	{
		...
		else if (item.GetItem() >= ITEMGET(0, 0) && item.GetItem() < ITEMGET(12, 0) && item.GetLevel() >= 4 && item.GetOption() >= 1)
		{
			price_tmp += item.GetOldPrice();
...
void MixHandler::ChaosMixWingThird(uint8 data)
{
	...
	if ( this->roll_success() )
	{
		...
		RandomValue<uint16> m_RandomValue(ITEMGET(12, 43)); //  Wing of Dimension SUM
		m_RandomValue.AddValue(ITEMGET(12, 36), 0); // Wing of Storm BK
		m_RandomValue.AddValue(ITEMGET(12, 37), 0); // Wing of Eternal SM 
		m_RandomValue.AddValue(ITEMGET(12, 38), 0); // Wing of Illusion ELF
		m_RandomValue.AddValue(ITEMGET(12, 39), 0); // Wing of Ruin MG
		m_RandomValue.AddValue(ITEMGET(12, 40), 0); // Cape of Emperor DL
		m_RandomValue.AddValue(ITEMGET(12, 43), 0); // Wing of Dimension SUM
		m_RandomValue.AddValue(ITEMGET(12, 50), 0); // Cape of Overrule RF
		m_RandomValue.AddValue(ITEMGET(12, 270), 0); // Cloak of Transcendence GL
		m_RandomValue.AddValue(ITEMGET(12, 467), 0); // Wings of Disillusion RW
		m_RandomValue.AddValue(ITEMGET(12, 472), 0); // Wings of Silence SL
		m_RandomValue.AddValue(ITEMGET(12, 489), 0); // Wings of Hit

		uint16 wing = m_RandomValue.GetRandomValue(RANDOM_POOL_RANDOM);
...
void MixHandler::ChaosMixSealedMasteryFinal(uint8 data)
{
	...
	if (this->roll_success())
	{
		...
		uint8 option = Random<uint8>(sGameServer->GetSealedMasteryAddOptionLevelMax());

		Item item_create(SealedMasteryID, 0, 0, 0, 0, 0, excellent, ancient);
		item_create.SetLuck(Random<uint8>(100) < sGameServer->GetSealedMasteryAddLuckRate());
		if (sGameServer->IsSealedMasteryAddExcellentEnabled()) {
			item_create.SetExe(GetRandomExcOption(sGameServer->GetRandomSealedMasteryExcellentOption()));
		}

		if (sGameServer->IsSealedMasteryAddOptionLevelEnabled()) {
			item_create.SetOption(option == 0 && option_rate < 4 ? 3 : option == 1 && option_rate < 8 ? 2 : option == 2 && option_rate < 12 ? 1 : 0);
		}
...
void MixHandler::ChaosMixUpgradeDivineArchangelWeapon(uint8 data)
{
	...
	if ( this->roll_success() )
	{
		if ( Excellent == 0 )
		{
			...
			Excellent = GetRandomExcOption(sGameServer->GetRandomUpgradeArchageWeaponOption());
		}

		if ( Luck == 0 )
		{
			Luck = roll_chance_i(75) ? 1 : 0;
		}
...
void MixHandler::ChaosMixUpgradeBloodangelToDarkangelWeapon(uint8 data)
{
	...
	chaos_box_loop(i)
	{
		...
		else if ( item->GetLevel() >= 11 && item->GetOption() >= 4 )
		{
			...
			else if (item->GetItem() == ITEMGET(0, 89))
			{
				Weapon = ITEMGET(0, 90);
			}
...
void MixHandler::ChaosMixUpgradeDarkangelToHolyangelWeapon(uint8 data)
{
	...
	chaos_box_loop(i)
	{
		...
		else if (item->GetLevel() >= 11 && item->GetOption() >= 4)
		{
			...
			else if (item->GetItem() == ITEMGET(0, 90))
			{
				Weapon = ITEMGET(0, 91);
			}
...
void MixHandler::ChaosMixUpgradeHolyangelToSoulWeapon(uint8 data)
{
	...
	chaos_box_loop(i)
	{
		...
		else if (item->GetLevel() >= 11 && item->GetOption() >= 4)
		{
			...
			else if (item->GetItem() == ITEMGET(0, 91))
			{
				Weapon = ITEMGET(0, 92);
			}
...
void MixHandler::ChaosMixUpgradeSoulToBlueEyeWeapon(uint8 data)
{
	...
	chaos_box_loop(i)
	{
		...
		else if (item->GetLevel() >= 11 && item->GetOption() >= 4)
		{
			...
			else if (item->GetItem() == ITEMGET(0, 92))
			{
				Weapon = ITEMGET(0, 93);
			}
...
void MixHandler::ChaosMixUpgradeBlueEyeToSilverHartWeapon(uint8 data)
{
	...
	chaos_box_loop(i)
	{
		...
		else if (item->GetLevel() >= 11 && item->GetOption() >= 4)
		{
			...
			else if (item->GetItem() == ITEMGET(0, 93))
			{
				Weapon = ITEMGET(0, 94);
			}
...
void MixHandler::ChaosMixCreateSpellStone(uint8 data)
{
	...
	if (!this->tributeTaxPrice() && type != SPIRIT_STONE_LESSER)
	{
		return;
	}
...
void MixHandler::ChaosMixBindWingsCreateLvl3(uint8 data)
{
	...
	if (this->roll_success())
	{
		...
		case Character::GUN_CRUSHER:
		{
									   		ConvertedWings = ITEMGET(12, 496); // [Bound] Wings of Hit
		} break;
		}
...
void MixHandler::ChaosMixBindWingsCreateLvl4(uint8 data)
{
	...
	chaos_box_loop(i)
	{
		...
		if (item->GetItem() == ITEMGET(12, 410)) // 12-410 no ?[Bound]Finished Condor's Magic Solution add omega
		{
			MagicSolution++;
		}
		else if (item->GetLevel() >= 9 && item->GetOption() >= 4)
		{
			...
			///- GC
			else if (item->GetItem() == ITEMGET(12, 489) || item->GetItem() == ITEMGET(12, 490))
			{
				Wings++;
				ConvertedWings = ITEMGET(12, 497);
			}
...
void MixHandler::ChaosMixFourthWings(uint8 data)
{
	...
	if (this->roll_success())
	{
		...
		RandomValue<uint16> m_RandomValue(ITEMGET(12, 414)); // Wings of Elysium SM   	  	 
		m_RandomValue.AddValue(ITEMGET(12, 414), 0); // Wings of Elysium SM
		m_RandomValue.AddValue(ITEMGET(12, 415), 0); // Wings of Flame God BK
		m_RandomValue.AddValue(ITEMGET(12, 416), 0); // Wing of Celestial Body ELF
		m_RandomValue.AddValue(ITEMGET(12, 417), 0); // Wings of Annihilation MG
		m_RandomValue.AddValue(ITEMGET(12, 418), 0); // Cloak of Dominator DL
		m_RandomValue.AddValue(ITEMGET(12, 419), 0); // Wings of Other World SUM
		m_RandomValue.AddValue(ITEMGET(12, 420), 0); // Cloak of Judgment RF
		m_RandomValue.AddValue(ITEMGET(12, 421), 0); // Cloak of Isolation GL
		m_RandomValue.AddValue(ITEMGET(12, 469), 0); // Wings of Destiny RW
		m_RandomValue.AddValue(ITEMGET(12, 474), 0); // Wings of Conviction SL
		m_RandomValue.AddValue(ITEMGET(12, 490), 0); // Heavenly Wings GC 

		uint16 item_create = m_RandomValue.GetRandomValue(RANDOM_POOL_RANDOM);

		Item itemrandom(item_create, 0, 0, 0, luck, option, exe, 0, socket);

		sItemMgr->ItemSerialCreateItem(this->GetPlayer(), serial_create_chaos_box, itemrandom, 1);
		this->ChaosMixLogSuccess();
...
void MixHandler::PetTrainerGuardianResurrection(uint8 type, uint8 id, uint8 main_slot, uint8 * slot, int32 slot_count)
{
	...
	if (this->roll_success())
	{
		...
		if (pItemInfo)
		{
			...
			if (pItemInfo->GetKind2() == ItemKind::GUARDIAN_MOUNT && option_count >= 3)
			{
				...
				else if (pMixGuardian->GetResultItem() == ITEMGET(12, 75) && roll_chance_i(sGameServer->GetRarePierceRyanRate())) {
	...
	else
	{
		...
		if (type == 2) {
			/*if (pMainItem->GetLevel() > 0 && pMainItem->GetLevel() <= 6) {
...
bool MixHandler::roll_success()
{
	...
	this->SetSuccess(roll_chance_i(this->success_rate.get() * 100, this->GetMaxSuccessRate() * 100));
...
//add UpgradeEarringHonorToBlood
