...
class MixMgr
{
	...
	public:
		...
		virtual ~MixMgr();
		void Clear();
		void LoadDisabled(char* pchFileName);
		void LoadSpellStone(char* pchFileName);
		void LoadGuardian(char* pchFileName);
		...
		MixGuardian const* GetGuardianMix2(uint8 type, uint8 id, uint32 MainItem) const;
