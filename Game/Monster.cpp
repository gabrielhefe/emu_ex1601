...
void Monster::AfterHitCheck(Unit * pAttacker, int32 damage, int32 shield_damage, uint16 damage_flag)
{
	...
	if ( pAttackerMaster && pAttackerMaster->IsPlayer() )
	{
		...
		if (sNoticeSystem->CheckMonsterKillNotice(this)) {
			sNoticeSystem->NoticeKillMonster(this->GetLastAttacker()->ToPlayer(), this);
		}
	}
...
void Monster::LogKillData(bool all, bool save_db)
{
	...
	if (all)
	{
		sObjectMgr->SendNoticeToAll(NOTICE_GLOBAL, "%s has been killed by %s", this->GetName(), this->GetLastAttacker()->GetName());
	}
	else
	{
		sObjectMgr->SendNoticeToMap(this->GetWorldId(), NOTICE_GLOBAL, "%s has been killed by %s", this->GetName(), this->GetLastAttacker()->GetName());
	}
}
