...
void Monster::DropItem()
{
	...
	uint16 RandomX = rand() % 2 + (-2);
	uint16 RandomY = rand() % 2 + (-2);
	...
	{
		sItemMgr->ItemSerialCreate(pMaxAttacker, this->GetWorldId(), this->GetX() + RandomX, this->GetY() + RandomY, Item(ITEMGET(14, 0), 0, 1.0f), true);
	}
	...
	if ( item_rate > 0 )
	{
		...
		if ( sHappyHour->GetState() == HAPPY_HOUR_STATE_START )
		{
			...
			ext_drop_per += sGameServer->happy_hour_drop_add.get();
		}
		...
		bool drop_ext = roll_chance_i(ext_drop_per * 100, 10000);
		...
		if (drop_ext)
		{
			...
			{
				item_drop = sMonsterManager->GenerateItem(item, this->GetLevel() - 15, true);
			}
		}
		else
		{
		...
		if ( item_drop )
		{
			...
			LOG_JEWEL_DROP(pMaxAttacker, this->GetWorldId(), this->GetX() + RandomX, this->GetY() + RandomY, &item);
				
			sItemMgr->ItemSerialCreate(pMaxAttacker, this->GetWorldId(), this->GetX() + RandomX, this->GetY() + RandomY, item, true);
			return;
		}
	}
...
void Monster::DropZen()
{
	...
	int16 randomX = rand() % 2 + (-2);
	int16 randomY = rand() % 2 + (-2);
	Player* pMaxAttacker = this->GetMaxAttacker();
	...
	if (RANDOM(zen_rate) < 10)
	{
		...
		pWorld->AddZen(this, this->GetX() + randomX, this->GetY() + randomY, zen_drop);
	}
	...
	if (RANDOM(400) == 1)
	{
		for (uint8 i = 0; i < 4; i++)
		{
			pWorld->AddZen(this, this->GetX() + randomX, this->GetY() + randomY, this->GetZen());
		}
	}
}