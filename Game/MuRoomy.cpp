...
void MuRoomy::LoadReward(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EventMuRoomyReward");

	uint32 count = 0;

	for (pugi::xml_node group = Main.child("Group"); group; group = group.next_sibling()) {
		for (pugi::xml_node data = group.child("Data"); data; data = data.next_sibling()) {
			MuRoomyReward* add_item = new MuRoomyReward;

			add_item->SetType(group.attribute("Type").as_int());
