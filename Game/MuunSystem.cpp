...
void MuunSystem::LoadIni() {
	if (boost::filesystem::exists(MUUN_FILEPATH)) {
		sGameServer->SetMuunAttackEnabled(GetPrivateProfileInt(MUUN_SECTION, "AttackEnabled", 0, MUUN_FILEPATH));
		sGameServer->SetMuunAttackDelay(GetPrivateProfileInt(MUUN_SECTION, "AttackDelay", 0, MUUN_FILEPATH));
		sGameServer->SetMuunAttackHitDelay(GetPrivateProfileInt(MUUN_SECTION, "AttackHitDelay", 0, MUUN_FILEPATH));
		sGameServer->SetMuunAttackRange(GetPrivateProfileInt(MUUN_SECTION, "AttackRange", 0, MUUN_FILEPATH));
		sGameServer->SetMuunDurabilityRate(GetPrivateProfileInt(MUUN_SECTION, "DurabilityRate", 0, MUUN_FILEPATH));
		sGameServer->SetMuunEvolveBonus(GetPrivateProfileInt(MUUN_SECTION, "EvolveBonus", 0, MUUN_FILEPATH));
		sGameServer->SetMuunEvolveBonusTime(GetPrivateProfileInt(MUUN_SECTION, "EvolveBonusTime", 0, MUUN_FILEPATH));
		sGameServer->SetMuunMountTime(GetPrivateProfileInt(MUUN_SECTION, "MountTime", 0, MUUN_FILEPATH));
	}
	else {
		sLog->outError(LOG_DEFAULT, "Error Load Muun.ini File");
		MessageBox(NULL, "Error Load Muun.ini File", "Error", ERROR);
		exit(0);
	}

}

void MuunSystem::Load(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("MuunList");

	uint32 count = 0;

	for (pugi::xml_node muun = Main.child("Muun"); muun; muun = muun.next_sibling()) {
		MuunSystemInfo* info = new MuunSystemInfo;

		info->SetIndex(muun.attribute("Item").as_int());
		info->SetRank(muun.attribute("Rank").as_int());
		info->SetOptionIndex(muun.attribute("Option").as_int());
		info->SetSpecialOptionType(muun.attribute("SpecialOptionType").as_int());
		info->SetSpecialOptionValue(muun.attribute("SpecialOptionValue").as_int());
		info->SetEvolutionItemIndex(muun.attribute("EvolutionItem").as_int());
...
void MuunSystem::LoadOption(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("MuunOption");
	...
	for (pugi::xml_node option = Main.child("Option"); option; option = option.next_sibling()) {
		...
		info->SetOptionValue(0, option.attribute("OptionValue1").as_int());
		info->SetOptionValue(1, option.attribute("OptionValue2").as_int());
		info->SetOptionValue(2, option.attribute("OptionValue3").as_int());
		info->SetOptionValue(3, option.attribute("OptionValue4").as_int());
		info->SetOptionValue(4, option.attribute("OptionValue5").as_int());
		info->SetMaxOptionValue(option.attribute("MaxOptionValue").as_int());
		info->SetMapZone(option.attribute("World").as_int());
		info->SetPlayTime(option.attribute("PlayTime").as_int());
		info->SetDayOfWeek(option.attribute("DayOfWeek").as_int());
		info->SetMinHour(option.attribute("StartHour").as_int());
		info->SetMaxHour(option.attribute("EndHour").as_int());
		info->SetMinLevel(option.attribute("MinLevel").as_int());
		info->SetMaxLevel(option.attribute("MaxLevel").as_int());
		info->SetMinMasterLevel(option.attribute("MinMasterLevel").as_int());
		info->SetMaxMasterLevel(option.attribute("MaxMasterLevel").as_int());
...
void MuunSystem::LoadEnergy(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("MuunEnergy");
	...
	for (pugi::xml_node option = Main.child("Option"); option; option = option.next_sibling()) {
		...
		info->SetRating(option.attribute("Rating").as_int());
		info->SetValue(0, option.attribute("Level1").as_int());
		info->SetValue(1, option.attribute("Level2").as_int());
		info->SetValue(2, option.attribute("Level3").as_int());
		info->SetValue(3, option.attribute("Level4").as_int());
		info->SetValue(4, option.attribute("Level5").as_int());
		info->SetEvolutionValue(option.attribute("Evolution").as_int());
...
void MuunSystem::LoadExchange(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("MuunExchange");

	uint32 count = 0;

	for (pugi::xml_node option = Main.child("Exchange"); option; option = option.next_sibling()) {
		MuunSystemExchange* info = new MuunSystemExchange;

		info->SetID(option.attribute("ID").as_int());
		info->SetCount(option.attribute("Count").as_int());
		info->SetMuun(option.attribute("MuunID").as_int());
...
void MuunSystem::GCMuunItemExpireSend(Player* pPlayer)
{
	...
	head->h.set(HEADCODE_MUUN_DATA, SUBCODE_MUUN_EXPIRE_SEND, sizeof(MUUN_EXPIRE_DATE_HEAD) + (sizeof(MUUN_EXPIRE_DATE_BODY) * head->count));
