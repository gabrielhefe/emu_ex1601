...
void NumericBaseball::LoadReward(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EventNumericBaseballReward");
	...
	for (pugi::xml_node data = Main.child("Data"); data; data = data.next_sibling()) {
		...
		pData->SetScoreMin(data.attribute("ScoreMin").as_int());
		pData->SetScoreMax(data.attribute("ScoreMax").as_int());
		pData->SetItem(data.attribute("ItemID").as_int());
