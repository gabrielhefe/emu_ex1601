...
void CObjectMgr::Init()
{
	...
	/*if (strcmp(sGameServer->GetNetworkIP(), "103.116.104.123") == 0) {
		this->SetMaxPlayers(MaxPlayersSet);
		this->SetReservedPlayers(ReservedPlayers);
		//MessageBox(NULL, "MaxPlayer", "MaxPlayer", 0);
	}
	else {
		//char ip[16];
		//sprintf(ip, sGameServer->GetNetworkIP());
		this->SetMaxPlayers(30);
		this->SetReservedPlayers(15);
		//MessageBox(NULL, ip, "LimitPlayer", 0);
	}*/

	this->SetMaxPlayers(MaxPlayersSet);
	this->SetReservedPlayers(ReservedPlayers);

	this->SetMaxMonsters(20000);
	this->SetMaxReservedMonsters(10000);
...
{
	/// - No deber�a pasar, pero hay que revisar que no sea 0
	if ( !id )
		return nullptr;

	PlayerMap::const_iterator it = this->m_player_accounts.find(id);

	/// - Busco si el jugador est� en la lista
...
/// - Esta funci�n no es seguro para usar std::map, queda normal
