		...
		case OBJECT_TYPE_PLAYER:
			{
				...
				sCustomRankTitle->SendRankTitleInfo(pMe, pPlayer);
			} break;
...
void Object::ViewportConstructPlayer(Object* pUnit, Player* pPlayer, uint8 * buffer, uint16 & size)
{
	...
	player_body.server = sGameServer->GetServerCode();
...
void Object::ViewportConstructPlayerChange(Object* pUnit, Player* pPlayer, uint8 * buffer, uint16 & size)
{
	...
	//body.preview[16] = pPlayer->GetInventory()->GetItem(7)->GetItem();
	...
	body.server = sGameServer->GetServerCode();
