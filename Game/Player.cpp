		...
		case 1: // Server Move Save
			{
				...
				{
					/*if (pPlayer->IsDueling()) {
						pPlayer->SendNotice(CUSTOM_MESSAGE_ID_BLUE, "Don't move server is dueling");
						return;
					}*/
					pPlayer->SendNotice(CUSTOM_MESSAGE_ID_BLUE, "Character saved. Moving to server.");

					SERVER_CHANGE_REQUEST pMsg;
					pMsg.server = pPlayer->GetDestServer(); // pMsg.wMapSvrCode = g_MapServerManager.GetMapSvrGroup();
		...
		case 2: // Character Logout
			{
				...
				{
					pPlayer->GetAccountData()->SetMoveToSelectChar(true);
		...
		if ( sGameServer->IsAttackSpeedCheck() )
		{
			...
			if ( att_speed < lpMsg->AttackSpeed || mc_speed < lpMsg->MagicSpeed )
			{
				...
				if ( attack_speed_diff >= sGameServer->GetAttackSpeedRange() || magic_speed_diff >= sGameServer->GetAttackSpeedRange() )
				{
					...
					{
						sLog->outError("antihack", "%s ClientHack Detected : Count Attack Speed Check: %d", this->BuildLog().c_str(), this->GetCounter(PLAYER_COUNTER_ATTACK_SPEED));
...
void Player::CharacterListRequestCallBack(QueryHolder holder)
{
	...
	if (sGameServer->GetServerType() == SERVER_TYPE_TEST) {
		if (!sMain->CheckAccountTestLoginSV(this->GetAccountData()->GetAccount())) {
			this->CloseSocket();
			return;
		}
	}

	if (!this->GetAccountData()->IsMoveToSelectChar()) {
		if (!CheckConnectAccountAllServer())
		{
			this->CloseSocket();
			return;
		}
	...
	this->GetAccountData()->SetMoveToSelectChar(false);
	...
	if ( character_result )
	{
		do
		{
			...
			this->GetCharacterList(characters)->SetResetTotal(fields[14].GetInt32());
	...
	for (uint8 i = 0; i < MAX_CHARACTER_PER_ACCOUNT; ++i) {
		if (!this->GetCharacterList(i))
			continue;

		uint16 guid = this->GetCharacterList(i)->GetGUID();

		if (guid == 0)
			continue;

		QueryResult item_equip = MuDatabase.PQuery("SELECT inventory FROM inventory_data WHERE id=%d", guid);

		if (item_equip) {
			Field* field = item_equip->Fetch();

			Tokenizer tokens1(field[0].GetBase64String().c_str(), ',');

			for (int j = 0; j < tokens1.size(); j++) {
				std::string buffer = tokens1[j];
				...
				std::string itemExe = tokens2[11];
				std::string itemAncient = tokens2[12];

				if (atoi(slot.c_str()) >= PLAYER_MAX_EQUIPMENT)
					break;

				this->GetCharacterList(i)->GetItem(atoi(slot.c_str()))->SetItem(atoi(itemEntry.c_str()));
				this->GetCharacterList(i)->GetItem(atoi(slot.c_str()))->SetLevel(atoi(itemLevel.c_str()));
				this->GetCharacterList(i)->GetItem(atoi(slot.c_str()))->SetExcellent(atoi(itemExe.c_str()));
				this->GetCharacterList(i)->GetItem(atoi(slot.c_str()))->SetAncient(atoi(itemAncient.c_str()));
			}
		}
	}
	...
	for ( uint8 i = 0; i < MAX_CHARACTER_PER_ACCOUNT; ++i )
	{
		...
		body[head->count].level = this->GetCharacterList(i)->GetLevelNormal();

		if (this->GetCharacterList(i)->GetLevelNormal() == 400) {
			body[head->count].level += this->GetCharacterList(i)->GetLevelMaster();
		}

		if (this->GetCharacterList(i)->GetLevelMaster() == 400 && this->GetCharacterList(i)->GetLevelNormal() == 400) {
			body[head->count].level += this->GetCharacterList(i)->GetLevelMajestic();
		}
...
void Player::CharacterListRequestNew()
{
	if ( this->GetConnectStatus() != CONNECT_STATUS_LOGGED )
	{
		this->CloseSocket();
	...
	QueryResult character_result = MuDatabase.PQuery("SELECT c.guid, c.name, c.level, c.level_master, c.level_majestic, c.authority, c.race, c.slot, g.ranking, g.guild_id, c.kick_time, "
		"c.pk_level, c.online, UNIX_TIMESTAMP(c.create_date), c.reset "
		"FROM character_info c LEFT JOIN guild_members g ON c.guid = g.char_id WHERE c.account_id = %d AND c.slot < %d ORDER BY c.last_use DESC", this->GetAccountData()->GetGUID(), MAX_CHARACTER_PER_ACCOUNT);
	...
	QueryResult account_data_result = MuDatabase.PQuery("SELECT a.vip_status, a.vip_duration, a.expanded_warehouse, a.expanded_warehouse_time, a.special_character "
		"FROM account_data a WHERE a.account_id = %d", this->GetAccountData()->GetGUID());
	...
	for (uint8 i = 0; i < MAX_CHARACTER_PER_ACCOUNT; ++i) {
		...
		QueryResult item_equip = MuDatabase.PQuery("SELECT InventoryData FROM character_item_inventory WHERE GUID=%d", guid);
		//QueryResult item_equip = MuDatabase.PQuery("SELECT inventory FROM inventory_data WHERE id=%d", guid);
		...
		if (item_equip) {
			...
			for (int j = 0; j < tokens1.size(); j++) {
				...
				std::string itemLevel = tokens2[6];
				std::string itemExe = tokens2[12];
				std::string itemAncient = tokens2[13];
...
void Player::CharacterSelect(uint8 * Packet, bool server_move)
{
	...
	this->CharacterSelectNew(character_name);
}
...
void Player::CharacterSelectNew(const char* charName)
{
	QueryResult result = MuDatabase.PQuery("SELECT c.guid FROM character_info c WHERE c.name = '%s' AND c.account_id = %d", charName, this->GetAccountData()->GetGUID());
	...
	this->CharacterLoginHandleNew(guid);
}
...
void Player::CharacterLoginHandle(QueryHolder holder)
{
	...
	if (this->IsFirstTime()) {
		sCharacterBase->GenerateCharacterBuffList(this);
		sCharacterBase->GenerateCharacterMoneyAdd(this);
	}
	...
	this->SetExpMurderPool(0);

	this->SetFirstTime(false);

	if ( !this->IsServerMove() )
	{
		if (strcmp(sGameServer->GetSecrecKeyLogin().c_str(), secreckey) != 0) {
			this->SaveCharacter();
			this->Close(6, CLOSE_FLAG_SELECT_CHAR);
		}
...
void Player::CharacterLoginHandleNew(uint32 charID)
{
	if ( this->GetConnectStatus() != CONNECT_STATUS_LOGGED )
	{
		this->SetAutoLogin(false);
...
void Player::CharacterMove(uint8 * Packet)
{
	...
	///- Si sos Admin y no est�s autorizado no pod�s caminar
	...
	if (this->GetPathData()->GetCount() > 0 && !this->IsOffline() && (this->GetRegenStatus() == REGEN_NONE) && !this->IsTeleporting() && (sGameServer->GetWalkSpeedCount() > 0))
	{
		...
		else
		{
			...
			if (this->IsAdministrator())
			{
				this->SendNotice(CUSTOM_MESSAGE_ID_BLUE, "Distance: %d - Speed Count: %d", walk_distance, this->GetWalkSpeedCount());
			}
			...
			if (walk_distance > sGameServer->GetWalkSpeedDistance())
			{
			...
			else if (walk_distance > sGameServer->GetWalkSpeedDistance2()) {
				sLog->outError("antihack", "%s -- %s [%d / %d] [%d / %d] : Distance: %d - Count: %d", __FUNCTION__, this->BuildLog().c_str(),
					this->GetACPathData()->GetX(), this->GetACPathData()->GetY(), start_x, start_y, walk_distance, this->GetWalkSpeedCount());
				...
				if (this->GetWalkSpeedCount() > sGameServer->GetWalkSpeedCount2())
				{
					if (sGameServer->IsWalkSpeedBan())
					{
						this->SetAuthority(1);
	...
	///- Este check habr�a que sacarlo o mejorarlo
	...
	///- Esto no deber�a ocurrir ni siquiera con lag, solo en caso de Cheat o p�rdida de Packets.
...
bool Player::ItemGet(uint8 * Packet, bool from_helper)
{
	...
	else if (world_item->item.IsEventArchangelWeapon())
	{
		...
		if (!pBloodCastle->CheckPicker(this)) {
			this->ItemGetResult();
			return false;
		}
	...
	if (sNoticeSystem->CheckItemNotice(world_item->item)) {
		sNoticeSystem->NoticeGetItem(this, world_item->item);
	}
...
void Player::ItemSell(uint8 * Packet)
{
	...
	if (sGameServer->IsSellNpcRecoveryEnabled()) {
...
void Player::protocol_core(uint8 * packet_buffer, uint16 size, uint8 headcode)
{
	TRY_START
		...
		if (sGameServer->IsDebug() )
		{
			sLog->outPacket("root", Packet, size, "ProtocolCore: ");
		}

		//MAKE_SUBCODE(subcode, Packet);
		//sLog->outError(LOG_DEFAULT, "headcode: %d - subcode: %d", headcode, subcode);
		...
		case HEADCODE_MISC_CHARACTER_DATA:
			{
				...
				case SUBCODE_MISC_CHARACTER_DATA_SECREC_KEY: { this->CheckSecrecKey(Packet); } break;
				case SUBCODE_MISC_CHARACTER_REQUEST_SV_DATA: { this->RequestSvDataFromChar(Packet); } break;
		...
		case HEADCODE_DUEL:
			{
				...
				switch(subcode)
				{
				case SUBCODE_DUEL_START_REQUEST: { this->DuelRequest(Packet); } break;
				case SUBCODE_DUEL_START_ANSWER: { this->DuelAnswer(Packet); } break;
				case SUBCODE_DUEL_END: { this->DuelEndRequest(Packet); } break;
				case SUBCODE_DUEL_LIFEBAR_INIT: { this->DuelSpectatorRequest(Packet); } break;
				case SUBCODE_DUEL_UNK_09: { this->DuelSpectatorRemove(Packet); } break;
				}
			} break;
		...
		case HEADCODE_MISC_DATA_ACTION:
			{
				MAKE_SUBCODE(subcode, Packet);

				switch( subcode )
				{
				case SUBCODE_MISC_DATA_JEWEL_INSERT: { sPentagramSystem->CGPentagramJewelInsertRecv(this, Packet); } break;
				case SUBCODE_MISC_DATA_PENTAGRAM_JEWEL_REMOVE: { sPentagramSystem->CGPentagramJewelRemoveRecv(this, Packet); } break;
				case SUBCODE_MISC_DATA_MIX_PENTAGRAM_RESULT: { sPentagramSystem->CGPentagramJewelRefineRecv(this, Packet); } break;
				case SUBCODE_MISC_DATA_JEWEL_UPGRADE: { sPentagramSystem->CGPentagramJewelUpgradeRecv(this, Packet); } break;
				case SUBCODE_MISC_DATA_PENTAGRAM_UPGRADE_RACIANCE_SLOT_RESULT: { sPentagramSystem->CGPentagramUpgradeRadianceSlot(this, Packet); } break;
				case SUBCODE_MISC_DATA_WING_SOCKET_OPTION_ADD_RESULT: { this->WingSocketOptionAdd(Packet); } break;
				case SUBCODE_MISC_DATA_WING_SOCKET_OPTION_UPGRADE_RESULT: { this->WingSocketOptionUpgrade(Packet); } break;
				case SUBCODE_MISC_DATA_EARRING_UPGRADE_RESULT:
					{
				...
				case SUBCOCE_MISC_DATA_HUNTING_RECORD:
					{
						sHuntingRecord->RecordRequest(this, Packet);
					} break;

				case SUBCOCE_MISC_DATA_HUNTING_RECORD_CLOSE:
					{
						sHuntingRecord->Close(this);
					} break;

				case SUBCODE_MISC_DATA_HUNTING_RECORD_SET_VISIBLE:
					{
						sHuntingRecord->SetVisible(this, Packet);
					} break;

				case SUBCODE_MISC_DATA_HUNTING_RIGHT_CLICK:
					{
						this->PlayerRightClick(Packet);
					} break;

				case SUBCODE_MISC_DATA_CHANNEL_WARP_RESULT:
					{
						this->ServerWarp(Packet);
					} break;

				case SUBCODE_MISC_DATA_SEND_SERVER_LIST:
					{
						this->SendServerList();
					} break;

				case SUBCODE_MISC_DATA_SEND_KUBERA_MINE_CART:
					{
		...
		case HEADCODE_MUUN_DATA:
			{
				MAKE_SUBCODE(subcode, Packet);

				switch( subcode )
				{
				case SUBCODE_MUUN_ITEM_GET: { sMuunSystem->CGMuunItemGetRecv(this, Packet); } break;
				case 0x01: { } break;
				case SUBCODE_MUUN_ITEM_USE: { sMuunSystem->CGMuunItemUseRecv(this, Packet); } break;
				case SUBCODE_MUUN_ITEM_SELL: { sMuunSystem->CGMuunItemSellRecv(this, Packet); } break;
				case 0x11: { sMuunSystem->CGMuunMountRecv(this, Packet, true); }break;
				case SUBCODE_MUUN_EXCHANGE: { sMuunSystem->CGMuunExchangeRecv(this, Packet); } break;
				case SUBCODE_MUUN_DISPLAY_SLOT_VIEWPORT: { sMuunSystem->SelectDisplayMuun(this, Packet); } break;
				}
			} break;
		...
		case HEADCODE_MINIMAP_DATA:
			{
				MAKE_SUBCODE(subcode, Packet);

				switch( subcode )
				{
				case SUBCODE_MINIMAP_DATA_PARTY_INFO: { sMiniMap->CGMiniMapStartPartyInfoRecv(this); } break;
				case SUBCODE_MINIMAP_DATA_INFO: { sMiniMap->CGMiniMapClosePartyInfoRecv(this); } break;
				case SUBCODE_MINIMAP_DATA_SKILL_DETECTION: { sMiniMap->CGMiniMapInfoRecv(this, Packet); } break;
				}
			} break;
		...
		case HEACODE_SOLDITEM_DATA:
			{
				MAKE_SUBCODE(subcode, Packet);

				switch( subcode )
				{
				case SUBCODE_SOLITEM_DATA_RECOVER_LIST_VIEW:
					{
				...
				case SUBCODE_SOLITEM_DATA_RECOVER_RESULT:
					{
		...
		case HEACODE_EVENT_INVENTORY:
			{
				MAKE_SUBCODE(subcode, Packet);

				switch( subcode )
				{
				case SUBCODE_EVENT_INVENTORY_ITEM_GET: { sEventInventory->CGEventItemGetRecv(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_ITEM_DROP_SEND: { sEventInventory->CGEventItemDropRecv(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_INVENTORY_OPEN_SEND: { sEventInventory->CGEventInventoryOpenRecv(this, Packet); } break;

				case SUBCODE_EVENT_INVENTORY_MU_ROOMY_START_REQUEST: { sMuRoomy->Start(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_MU_ROOMY_REVEAL_CARD_RESULT: { sMuRoomy->Reveal(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_MU_ROOMY_CARD_MOVE_RESULT: { sMuRoomy->Move(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_MU_ROOMY_CARD_REMOVE_RESULT: { sMuRoomy->Remove(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_MU_ROOMY_MATCH_RESULT: { sMuRoomy->Match(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_MU_ROOMY_END_RESULT: { sMuRoomy->End(this, Packet); } break;
				case 0x19: { sMuRoomy->SpecialMatch(this, Packet); } break;

				case SUBCODE_EVENT_INVENTORY_MINI_BOMB_START_RESULT: { sMiniBomb->Start(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_MINIBOMB_REVEAL: { sMiniBomb->Reveal(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_MINI_BOMB_MARK_RESULT: { sMiniBomb->Mark(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_MINIBOMB_OPEN: { sMiniBomb->Open(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_MINI_BOMB_GIFT_RESULT: { sMiniBomb->Reward(this, Packet); } break;

				case SUBCODE_EVENT_INVENTORY_JEWEL_BINGO_STATE: { sJewelBingo->Start(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_JEWEL_BINGO_INFO: { sJewelBingo->Move(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_JEWEL_BINGO_BOX_SELECT: { sJewelBingo->BoxSelect(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_JEWEL_BINGO_PLAY_INFO: { sJewelBingo->JewelSelect(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_JEWEL_BINGO_PLAY_RESULT: { sJewelBingo->Reward(this, Packet); } break;

				case SUBCODE_EVENT_INVENTORY_NUMERIC_BASEBALL_START: { sNumericBaseball->Start(this, Packet); } break;
				case SUBCODE_EVENT_INVENTORY_NUMERIC_BASEBALL_0x4D_0x31: { sNumericBaseball->SelectNumber(this, Packet); } break;
				}
			} break;

		case HEADCODE_GUILD_MATCHING:
			{
				MAKE_SUBCODE(subcode, Packet);

				switch( subcode )
				{
				case SUBCODE_GUILD_MATCHING_LIST: { sGuildMatching->CGGuildMatchingListRecv(this, Packet); } break;
				case SUBCODE_GUILD_MATCHING_LIST_SEARCH: { sGuildMatching->CGGuildMatchingListSearchRecv(this, Packet); } break;
				case SUBCODE_GUILD_MATCHING_REGISTER_RESULT: { sGuildMatching->CGGuildMatchingInsertRecv(this, Packet); } break;
				case SUBCODE_GUILD_MATCHING_CANCEL_REGISTER_RESULT: { sGuildMatching->CGGuildMatchingCancelRecv(this, Packet); } break;
				case SUBCODE_GUILD_MATCHING_JOIN_RESULT: { sGuildMatching->CGGuildMatchingJoinInsertRecv(this, Packet); } break;
				case SUBCODE_GUILD_MATCHING_JOIN_CANCEL_RESULT: { sGuildMatching->CGGuildMatchingJoinCancelRecv(this, Packet); } break;
				case SUBCODE_GUILD_MATCHING_JOIN_ACCEPT_RESULT: { sGuildMatching->CGGuildMatchingJoinAcceptRecv(this, Packet); } break;
				case SUBCODE_GUILD_MATCHING_JOIN_LIST: { sGuildMatching->CGGuildMatchingJoinListRecv(this, Packet); } break;
				case SUBCODE_GUILD_MATCHING_JOIN_INFO_RESULT: { sGuildMatching->CGGuildMatchingJoinInfoRecv(this, Packet); } break;
				}
			} break;

		case HEADCODE_PARTY_MATCHING:
			{
				MAKE_SUBCODE(subcode, Packet);

				switch( subcode )
				{
				case SUBCODE_PARTY_MATCHING_REGISTER_RESULT: { sPartyMatching->HandleRegister(this, Packet); } break;
				case SUBCODE_PARTY_MATCHING_HANDLE_SEARCH: { sPartyMatching->HandleSearch(this, Packet); } break;
				case SUBCODE_PARTY_MATCHING_JOIN_RESULT: { sPartyMatching->HandleJoin(this, Packet); } break;
				case SUBCODE_PARTY_MATCHING_JOIN_DATA_RESULT: { sPartyMatching->HandleJoinData(this, Packet); } break;
				case SUBCODE_PARTY_MATCHING_JOIN_LIST: { sPartyMatching->HandleJoinList(this, Packet); } break;
				case SUBCODE_PARTY_MATCHING_ACCEPT_RESULT: { sPartyMatching->HandleAccept(this, Packet); } break;
				case SUBCODE_PARTY_MATCHING_CANCEL_RESULT: { sPartyMatching->HandleCancelRegister(this, Packet); } break;
				case SUBCODE_PARTY_LEADER_CHANGE_RESULT: { this->PartyLeaderChange(Packet); }break;
				}
			} break;

		case HEADCODE_CHAT_BLOCK:
			{
				MAKE_SUBCODE(subcode, Packet);
		...
		case HEADCODE_MERCHANT_DATA:
			{
				MAKE_SUBCODE(subcode, Packet);

				switch(subcode)
				{
				case SUBCODE_MERCHANT_DATA_DARK_ANCIENT_ITEM_CREATE:
				case SUBCODE_MERCHANT_DATA_DARK_DARK_ANCIENT_ITEM_DESTROY:
					{
		...
		case HEADCODE_SOCKET_MIX_RESULT:
			{
		...
		case HEADCODE_ITEM_ACTION:
			{
				MAKE_SUBCODE(subcode, Packet);

				switch (subcode)
				{
				case SUBCODE_ITEM_ACTION_SPLIT_RESULT:
					this->ItemSplit(Packet);
					break;

				case SUBCODE_ITEM_ACTION_ENHANCEMENT_RESULT:
					this->ItemEnhancementRequest(Packet);
					break;
				}
			} break;

		case HEADCODE_BASIC_DATA:
			{
				MAKE_SUBCODE(subcode, Packet);

				switch (subcode)
				{
				case SUBCODE_BASIC_DATA_POSITION_ATTACK_RESULT:
					{
						this->PositionAttack(Packet);
					} break;

				case SUBCODE_BASIC_DATA_WARP_FAVORITE_LIST:
					{
						this->WarpFavoriteListSave(Packet);
					} break;
				}
		...
		case HEADCODE_PERSONAL_STORE:
		{
			MAKE_SUBCODE(subcode, Packet);

			switch (subcode)
			{
			case SUBCODE_PERSONAL_STORE_LIST:
			{
				this->HandlePersonalStoreSearch(Packet);
			} break;

			case SUBCODE_PERSONAL_STORE_ITEM_LIST:
			{
				this->HandlePersonalStoreSearchItemList(Packet);
			} break;

			case SUBCODE_PERSONAL_STORE_VIEW:
			{
				this->HandlePersonalStoreView(Packet);
			} break;

			case SUBCODE_PERSONAL_STORE_SEARCH_ITEM:
			{
				this->HandlePersonalStoreSearchItem(Packet);
			} break;

			case SUBCODE_PERSONAL_STORE_SELECT_ITEM:
			{
				this->HandlePersonalStoreSelectItem(Packet);
			} break;

			case SUBCODE_PERSONAL_STORE_PURCHASE_RESULT:
			{
				this->HandlePersonalStorePurchase(Packet);
			} break;

			case SUBCODE_PERSONAL_STORE_SELL_LIST:
			{
				this->HandlePersonalStoreSellList(Packet);
			} break;

			case SUBCODE_PERSONAL_STORE_SELL_ITEM_VIEW:
			{
				this->HandlePersonalStoreSellItemView(Packet);
			} break;

			case SUBCODE_PERSONAL_STORE_SELL_ITEM_RESULT:
			{
				this->HandlePersonalStoreSellItem(Packet);
			} break;

			case SUBCODE_PERSONAL_STORE_SELL_ITEM_CANCEL:
			{
				this->HandlePersonalStoreSellItemCancel(Packet);
			} break;

			case SUBCODE_PERSONAL_STORE_STATE:
			{
...
bool Player::SecureCodeCheck(const char * code) const
{
	...
	if ( !memcmp(this->GetAccountData()->secure_code, code, MAX_SECURE_CODE_LENGTH) || strcmp(this->GetAccountData()->secure_code, code) == 0)
		return true;
...
void Player::AutoLoginCharacterSelect()
{
	...
	this->CharacterLoginHandleNew(this->GetAutoLoginCharacter());
}
...
bool Player::CheckConnectAccountAllServer() {
	if (sMain->CheckAccountTestLoginSV(this->GetAccountData()->GetAccount())) {
		return true;
	}

	QueryResult countip = MuDatabase.PQuery("SELECT COUNT(*) FROM account_data WHERE current_ip='%s' and current_server!=65535", this->GetAccountData()->GetIP());

	if (countip) {
		Field* reader = countip->Fetch();

		if (reader[0].GetInt8() >= sGameServer->GetMaxConnectionsPerIP()) {
			sLog->outError(LOG_NETWORK, "IP connection count max: %s", this->GetAccountData()->GetIP());
			return false;
		}
	}

	if (sGameServer->IsCheckConnectAccountMacEnable()) {
		QueryResult countmac = MuDatabase.PQuery("SELECT COUNT(*) FROM account_data WHERE current_mac='%s' and current_server!=65535", this->GetAccountData()->GetMac());

		if (countmac) {
			Field* reader = countmac->Fetch();

			if (reader[0].GetInt8() >= sGameServer->GetMaxConnectionPerMAC()) {
				sLog->outError(LOG_NETWORK, "MAC connection count max: %s", this->GetAccountData()->GetMac());
				return false;
			}
		}
	}

	if (sGameServer->IsCheckConnectAccountDiskSerialEnable()) {
		QueryResult countdiskserial = MuDatabase.PQuery("SELECT COUNT(*) FROM account_data WHERE current_diskserial=%u and current_server!=65535", this->GetAccountData()->GetDiskSerial());

		if (countdiskserial) {
			Field* reader = countdiskserial->Fetch();

			if (reader[0].GetInt8() >= sGameServer->GetMaxConnectionPerDiskSerial()) {
				sLog->outError(LOG_NETWORK, "Disk Serial connection count max: %u", this->GetAccountData()->GetDiskSerial());
				return false;
			}
		}
	}
