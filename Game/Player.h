...
class Player: public Unit
{
	...
	public:
		...
		void CheckOfflineAttack(const char* account);
		...
		void CharacterListRequestNew();

		bool CheckConnectAccountAllServer();
		...
		void CharacterSelectNew(const char* charName);
		void CharacterSelectCallBack(PreparedQueryResult result);
		void CharacterLoginHandleNew(uint32 charID);
		...
		int32 GetReset() const { return this->GetResetSystem(RESET_TOTAL)->GetReset(); }
		...
		void CheckSecrecKey(uint8* Packet);

		DWORD EventTickSendRequest;
		void RequestSvDataFromChar(uint8* Packet);
	...
	// Save to DB
		...
		void SaveGremoryCaseAccBinary();
		void SaveGremoryCaseMobileBinary();
		void SaveGremoryCaseCharaBinary();
		void SaveGremoryCasePSBinary();
		void SaveDBAllItemBox();
		// Send To Client
		void SendJoinInfo();
	...
	// Season 10 and later Exp Formula
		...
		void UpdateCharRankingInfo(Player* pPlayer);
	...
	// Cash Shop
		...
		void LoadCashShopcoin();

		void CashShopGiftSendNew(const char* Name);
	...
	public:
		...
		DECLARE_PROPERTY_ARRAY(int64, Stat, MAX_STAT_TYPE);
		DECLARE_PROPERTY_ARRAY(int64, AddStat, MAX_STAT_TYPE);
		...
		DECLARE_ARRAY_STRUCT(BetItemData, BetItemData, BET_MAX);
		DECLARE_ARRAY_STRUCT(DuelInfoData, DuelInfoData, DUEL_SCORE_MAX);
		DECLARE_ARRAY_STRUCT(RankLevelUserData, RankLevelUserData, RANKLEVELMAX);
		...
		DECLARE_ENUM(uint8, MonsterSoulPurchase);
		...
		DECLARE_ENUM(int64, ExpMurderPool);
