...
void Player::CashShopGiftSendNew(const char* Name)
{
	...
	QueryResult result = MuDatabase.PQuery("SELECT guid, account_id, level FROM character_info WHERE name = '%s'", Name);
...
void Player::LoadCashShopGiftAcc()
{
	this->SetGiftQuery(false);
	...
	QueryResult result = MuDatabase.PQuery("SELECT a.product, a.option, a.serial, a.serial_cash_shop, a.server, a.date "
		"FROM account_cash_shop_gift a WHERE a.account_id = %d", this->GetAccountData()->GetGUID());
