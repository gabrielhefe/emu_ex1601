...
void Player::ClearCharacter()
{
	...
	this->GetResetSystem(RESET_TOTAL)->SetReset(0);
...
void Player::CalculateCharacter()
{
	...
	/*QueryResult result = MuDatabase.PQuery("SELECT * FROM character_add_stat WHERE GUID=%d", this->GetGUID());

	if (result) {
		do {
			Field* fields = result->Fetch();

			this->IncreaseAddStat(STRENGTH, fields[1].GetUInt32());
			this->IncreaseAddStat(AGILITY, fields[2].GetUInt32());
			this->IncreaseAddStat(VITALITY, fields[3].GetUInt32());
			this->IncreaseAddStat(ENERGY, fields[4].GetUInt32());

			if (this->GetClass() == Character::DARK_LORD) {
				this->IncreaseAddStat(LEADERSHIP, fields[5].GetUInt32());
			}
		} while (result->NextRow());
	}
	else {
		QueryResult result = MuDatabase.PQuery("INSERT INTO character_add_stat(GUID, Strenght, Agility, Vitality, Energy, Leadership) VALUES(%d, 0, 0, 0, 0, 0)", this->GetGUID());
	}*/
	...
	*/
	...
	if ( right && left )
	{
		...
		this->IncreaseIntData(UNIT_INT_ATTACK_SPEED, ((Right->GetAttackSpeed() + Left->GetAttackSpeed())) * 75 / 100);
		this->IncreaseIntData(UNIT_INT_MAGIC_SPEED, ((Right->GetAttackSpeed() + Left->GetAttackSpeed())) * 75 / 100);
	}
	else if ( right )
	{
...
void Player::CalculateExeItem()
{
	wear_inventory_loop(i)
	{
		...
		if (!pItem->CheckIsArchngelWeapon()) {
...
void Player::ApplyStats1()
{
	...
	if (pHelper && pHelper->IsItem() && pHelper->IsValid() && !pHelper->IsExpired())
	{
		...
		if (pHelper->GetItem() == PET_SKELETON || pHelper->GetItem() == ITEMGET(13, 470)) {
			this->IncreaseData(PLAYER_DATA_ADD_EXPERIENCE_RATE, 20);
		}
...
void Player::ApplyEarringOption(uint8 level, uint8 group, uint8 option, uint8 value, bool stat)
{
	...
	case 11:
		{
			this->PowerIncreseAdd(POWER_LIFE, pEarringValue->GetValue(), true);
		} break;
...
{
	int playerLevelNormal = GetLevelData(LEVEL_DATA_NORMAL)->GetLevel();
	if (playerLevelNormal == 400) {
...
{
	int playerLevelNormal = GetLevelData(LEVEL_DATA_NORMAL)->GetLevel();
	int playerLevelMaster = GetLevelData(LEVEL_DATA_MASTER)->GetLevel();
	if (playerLevelNormal == 400 && playerLevelMaster == 400) {
...
bool Player::UpgradeItemUsingMysteriousStone(uint8 source_slot, uint8 target_slot)
{
	...
	if (target_item->GetItem() < ITEMGET(6, 0))
	{
		...
		target_item->SetExe(GetRandomExcOption(sItemMgr->m_mysteriouse_stone_data[1].GetRandomAddExl()));
		target_item->SetLuck(roll_chance_i(sItemMgr->m_mysteriouse_stone_data[1].GetLuckRate()) ? 1 : 0);
		modified = true;
	...
	else if (target_item->GetItem() < ITEMGET(7, 0))
	{
		...
		target_item->SetExe(GetRandomExcOption(sItemMgr->m_mysteriouse_stone_data[2].GetRandomAddExl()));
		target_item->SetLuck(roll_chance_i(sItemMgr->m_mysteriouse_stone_data[2].GetLuckRate()) ? 1 : 0);
		modified = true;
	...
	else if (target_item->GetItem() < ITEMGET(12, 0))
	{
		if (target_item->GetAncient())
		{
			...
			bool getorancient = roll_chance_i(sItemMgr->m_mysteriouse_stone_data[0].GetAncient2Rate()) ? 1 : 0;
			if (getorancient) {
				target_item->OrAncient(8);
			}
			else {
				target_item->OrAncient(4);
			}
			target_item->SetLuck(roll_chance_i(sItemMgr->m_mysteriouse_stone_data[0].GetLuckRate()) ? 1 : 0);
			target_item->SetExe(0);
			modified = true;
		}
		else {
			if (Random(10000) <= sItemMgr->m_mysteriouse_stone_data[0].GetUpAncientRate()) {
				target_item->SetAncient(1);
				target_item->SetExe(0);
				modified = true;
...
bool Player::MoveToGate(uint16 gateId)
{
	...
	if (!this->GetBetItemData(BET_ITEM_MAX)->IsDuelFinishTrade()) {
		if (this->GetInterfaceState()->GetID() != InterfaceData::None)
		{
...
void Player::GiveSingleExperience(Monster * mMonster, int32 damage)
{
	...
	int64 experience = level + max_level;

	experience = damage * experience / mMonster->PowerGetTotal(POWER_LIFE);

	if (sGameServer->IsDynamicExperience()) {

		Player* pPlayer = sObjectMgr->FindPlayerByName(this->GetName());
		experience *= sDynamicExperienceMgr->ExpToPlayer(pPlayer);
	}
	else {
...
void Player::GiveSingleExperienceFormula(Monster* mMonster, int32 damage)
{
	...
	int16 exp_dec_rate = rand() % 100 + 95;
	experience = experience * pWorld->GetExperienceRate() / exp_dec_rate;
...
void Player::GivePartyExperienceFormula(Monster* mMonster, int32 damage)
{
	...
	for (uint8 i = 0; i < MAX_PARTY_MEMBERS; ++i)
	{
		...
		if (!sGameServer->IsPartyKeepExpSolo()) {
			experience = sFormulaMgr->GetValue(0, 4, (uint32)solo_exp, (float)(viewpercent / 100), pPlayer->GetTotalLevel(), total_level) * (0.9f + (0.1f * viewplayer));
		}
		else {
			experience = solo_exp * viewpercent / 100;
		}
		//experience = solo_exp * viewpercent / 100 / viewplayer;

		int16 m_zen_random_rate = rand() % 5 + 1;
		m_zen = m_zen / m_zen_random_rate;
...
{
	sResetSystemMgr->AutoReset(this);
	...
	if (this->GetPKLevel() > 3) {
		uint64 expNeed = sGameServer->GetExpPoolMuderPerLevel() * level;
		this->SetExpMurderPool(this->GetExpMurderPool() + experience);

		int64 getexppool = this->GetExpMurderPool();

		if (getexppool >= expNeed) {
			this->SetPKLevel(this->GetPKLevel() - 1);
			this->SetPKCount(this->GetPKCount() - 1);
			if (this->GetPKLevel() == PK_STATUS_COMMONER || this->GetPKPoints() == 0) {
				this->SetPKPoints(0);
			}
			else {
				this->SetPKPoints(this->GetPKPoints() / 2);
			}
			this->SetExpMurderPool(0);
			this->PKLevelSend();
		}
	...
	if (this->GetLevelData(LEVEL_DATA_NORMAL)->GetLevel() == 400) {
		if (this->GetLevelData(LEVEL_DATA_MASTER)->GetLevel() == 0 && this->GetLevelData(LEVEL_DATA_MASTER_BAK)->GetLevel() > 0) {
		...
		if (this->GetLevelData(LEVEL_DATA_MASTER)->GetLevel() == 400 && this->GetLevelData(LEVEL_DATA_MAJESTIC)->GetLevel() == 0 && this->GetLevelData(LEVEL_DATA_MAJESTIC_BAK)->GetLevel() > 0) {
			this->GetLevelData(LEVEL_DATA_MAJESTIC)->SetLevel(this->GetLevelData(LEVEL_DATA_MAJESTIC_BAK)->GetLevel());
			this->GetLevelData(LEVEL_DATA_MAJESTIC)->SetPoints(this->GetLevelData(LEVEL_DATA_MAJESTIC_BAK)->GetPoints());
			this->GetLevelData(LEVEL_DATA_MAJESTIC_BAK)->SetLevel(0);
			this->MasterSendLevelUp();
		}
	...
	this->UpdateCharRankingInfo(this);
...
void Player::UpdateCharRankingInfo(Player* pPlayer) {

	if (!pPlayer)
		return;

	QueryResult update_ranking = MuDatabase.PQuery("REPLACE INTO server_ranking(guid, race, level_normal, level_master, level_majestic, reset, update_time) VALUES(%d,%d,%d,%d,%d,%d,NOW())", pPlayer->GetGUID(), pPlayer->GetDBClass(), pPlayer->GetLevelData(LEVEL_DATA_NORMAL)->GetLevel(),
		pPlayer->GetLevelData(LEVEL_DATA_MASTER)->GetLevel(), pPlayer->GetLevelData(LEVEL_DATA_MAJESTIC)->GetLevel(), pPlayer->GetResetSystem(RESET_TOTAL)->GetReset());
...
void Player::LevelUpAddPoints()
{
	...
	else
	{
		...
		if ( this->GetDBClass() == DB_CLASS_SOUL_MASTER || this->GetDBClass() == DB_CLASS_BLADE_KNIGHT || this->GetDBClass() == DB_CLASS_MUSE_ELF || 
			this->GetDBClass() == DB_CLASS_BLOODY_SUMMONER || this->GetDBClass() == DB_CLASS_RUNE_SPELL_MASTER || this->GetDBClass() == DB_CLASS_ROYAL_SLAYER || this->GetDBClass() == DB_CLASS_GUNNER_2)
		{
			this->GetLevelData(LEVEL_DATA_NORMAL)->IncreasePoints(base_info->GetPlusLevelUpPoints());
		}
	}
...
bool Player::WearItem(Item const* mItem, uint8 slot)
{
	...
	if ( slot == RING_01 || slot == RING_02 ) // Verificar que no se puedan usar 2 rings de transformaci�n
	{
...
void Player::PreviewMake()
{
	...
	case ITEMGET(12, 489):
	{
		//this->preview[16].set(pWings->GetItem());
		this->preview[16].op_or(4);
		this->preview[9].op_or(1);
	} break;
...
void Player::ChatBlockList(uint8 type, uint8 result)
{
	...
	head->h.set(HEADCODE_CHAT_BLOCK, 0x00, sizeof(CHAT_BLOCK_LIST_HEAD) + (head->count * sizeof(CHAT_BLOCK_LIST_BODY)));
...
void Player::ItemSoldListView()
{
	...
	head->h.set(HEACODE_SOLDITEM_DATA, SUBCODE_SOLITEM_DATA_RECOVER_LIST_VIEW, sizeof(SOLD_ITEM_LIST_HEAD) + (sizeof(SOLD_ITEM_LIST_BODY) * count));
...
void Player::SendServerList()
{
	...
	if (sGameServer->GetServerType() == SERVER_TYPE_NORMAL && sGameServer->IsFlag(SERVER_FLAG_DISPLAY) && sGameServer->IsChannelChangeEnabled())
	{
		for (ChannelDataMap::const_iterator itr = sConnectServer->m_ChannelDataMap.begin(); itr != sConnectServer->m_ChannelDataMap.end(); ++itr)
		{
			...
			if (pData->GetServer() == sGameServer->GetServerTestCodeIndex())
				continue;
	...
	head->Set(HEADCODE_MISC_DATA_ACTION, 0x57, sizeof(PACKET_CHANNEL_HEAD) + (head->count * sizeof(PACKET_CHANNEL_BODY)));
...
void Player::ServerWarp(uint8 * Packet)
{
	...
	if (sGameServer->IsChannelChangeNotUseHighPKEnabled()) {
		uint8 high_pk_level = sGameServer->GetChannelChangeNotUseHighPKLevel() + 3;

		if (high_pk_level == 3) {
			high_pk_level = 4;
		}
		else if (high_pk_level > 6) {
			high_pk_level = 6;
		}

		if (this->GetPKLevel() >= high_pk_level) {
			this->SendNotice(CUSTOM_MESSAGE_ID_RED, "Pk too high could not be used this feature ");
			this->SendPacket(&pMsg);
			return;
		}
	}
	...
	//server = lpMsg->server - MAX_SERVER_PER_GROUP;
	server = lpMsg->server;
...
void Player::CheckSecrecKey(uint8* Packet) {
	POINTER_PCT_CHECK(PACKET_SECRECKEY_CLIENT, lpMsg, Packet, 0);
	sprintf(secreckey, lpMsg->secrectkey);
}

void Player::RequestSvDataFromChar(uint8* Packet) {
	if ((GetTickCount() - EventTickSendRequest) < 5000)
		return;

	EventTickSendRequest = GetTickCount();

	POINTER_PCT_CHECK(PACKET_REQUEST_SERVER_DATA, lpMsg, Packet, 0);

	if (lpMsg->type == 0) {
		sCustomRankServer->SendRankServerData(this);
	}
...
bool Player::IsSkillCPS(uint16 skill, uint32 time)
{
	...
	else
	{
		...
		if (skill == 0) {
			if (diff < sGameServer->GetNormalAttackDiffCheck()) {
				...
				{
					sGameServer->LogAntiHack(this, ANTIHACK_ID_ATTACK_TIME, "Skill: %u - Speed: %d - Count: %d - Diff: %I64d", skill, this->GetIntData(UNIT_INT_ATTACK_SPEED), pSkillCPS->GetCount(), diff);
				}

				if (sGameServer->GetAntiHackSkillCount() > 0 && pSkillCPS->GetCount() >= sGameServer->GetAntiHackSkillCount())
				{
					if (sGameServer->IsAntiHackSkillBan()) {
						this->SetAuthority(1);
					}
...
void Player::ItemEnhancementRequest(uint8 * Packet)
{
	...
	if (roll_chance_i(success_rate * 100, 10000))
	{
		pMsg.result = 0;
		...
		if (pItem->GetLevel() >= sGameServer->GetNoticeEnhancementSuccesMinLevel()) {
			if (sGameServer->IsNoticeEnhancementSuccesAllEnable()) {
	...
	else
	{
		...
		if (pItem->GetLevel() > 0)
		{
			...
			else if (pTalismanChaos) {
				pItem->common.talisman = 0;

				pItem->SetLevel(pItem->GetLevel() - 2);
				pItem->AlterDurability(3, 0.0f);
				pItem->Convert();
