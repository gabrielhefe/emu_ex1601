...
void Player::SelectDBFriendAdd() {
	if (!Player::IsPlayerBasicState(this, false, false))
	{
		return;
	}

	QueryResult result = MuDatabase.PQuery("SELECT guid, online, server_code, authority FROM character_info WHERE name = '%s'", this->requested_friend);
...
void Player::SelectDBFriendMail() {
	...
	QueryResult result = MuDatabase.PQuery("SELECT guid, account_id, level FROM character_info WHERE name='%s'", this->requested_friend);
...
void Player::MailNewUpdate()
{
	...
	this->MailNewUpdateDB();
}

void Player::MailNewUpdateDB() {
	if (!Player::IsPlayerBasicState(this, false, false))
	{
		return;
	}
	...
	QueryResult result = MuDatabase.PQuery("SELECT c.from_id, c.subject, c.message, c.direction, c.action, c.photo, c.window_guid, c.date, c.opened, c.id FROM character_mail c WHERE c.char_id = %d AND c.opened = %d", this->GetGUID(), FRIEND_MAIL_STATE_NEW);
