...
bool Player::HelperCanLoot(WorldItem const* pWorldItem)
{
	...
	///- Si el helper est� configurado para agarrar todos los items, entonces no lo proceso
	if (this->GetHelper()->IsOptPickAllNearItems())
	{
	...
	///- Si el helper est� configurado para agarrar solo algunos items, entonces lo proceso
	if ( this->GetHelper()->IsOptPickSelectedItems() )
	{
...
void Player::HelperAutoloot()
{
	...
	///- Si no tiene ninguna opci�n de loot, entonces no lo proceso
	if ( !this->GetHelper()->IsOptPickAllNearItems() && !this->GetHelper()->IsOptPickSelectedItems() )
	{
		return;
	}

	if (this->GetHelper()->IsOffline() && !sGameServer->IsOfflineAttackDetailAutoloot())
		return;
	...
	for ( ViewportDataList::const_iterator it = viewport.begin(); it != viewport.end(); ++it )
	{
		...
		if ( !data ) ///- Si la data no existe por alg�n motivo, no lo proceso
			continue;

		if ( data->GetStatus() != Viewport::STATUS_READY ) ///- Solamente proceso objetos que est�n listos
			continue;
		...
		if ( this->HelperCanLoot(pWorldItem) )
		{
			...
			case ITEM_INVENTORY_TYPE_NORMAL:
				{
					ITEM_GET pMsg;
					pMsg.h.set(HEADCODE_ITEM_USE, sizeof(ITEM_GET));
