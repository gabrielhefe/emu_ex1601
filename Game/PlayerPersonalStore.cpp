...
void Player::HandlePersonalStoreView(uint8 * Packet)
{
	...
	head->Set(HEADCODE_PERSONAL_STORE, SUBCODE_PERSONAL_STORE_VIEW, sizeof(PERSONAL_STORE_VIEW_HEAD)+(sizeof(PERSONAL_STORE_VIEW_BODY)* head->count));
...
void Player::HandlePersonalStoreSelectItem(uint8 * Packet)
{
	...
	head->Set(HEADCODE_PERSONAL_STORE, SUBCODE_PERSONAL_STORE_SELECT_ITEM, sizeof(PERSONAL_STORE_SELECT_ITEM_HEAD)+(sizeof(PERSONAL_STORE_SELECT_ITEM_BODY)* head->count));
	this->sendPacket(buffer);
...
void Player::HandlePersonalStorePurchase(uint8 * Packet)
{
	...
	//char* seller_name = lpHead->seller;
	std::string sellername = lpHead->seller;
	STRING_SAFE_COPY(seller_name, MAX_CHARACTER_LENGTH + 1, sellername.c_str(), sellername.size());
	Player* pPlayer = sObjectMgr->FindPlayerByName(seller_name);

	/*if (!pPlayer) {
		this->SendNotice(CUSTOM_MESSAGE_ID_BLUE, "Seller: Null", seller_name);
		return;
	}
	else if (!pPlayerSell) {
		this->SendNotice(CUSTOM_MESSAGE_ID_BLUE, "pPlayerSellSeller: Null", seller_name);
		return;
	}
	else {
		this->SendNotice(CUSTOM_MESSAGE_ID_BLUE, "Seller: %s", pPlayerSell->GetName());
		return;
	}*/
	...
	{
		if (sGameServer->IsPersonalStoreCreditsPurcharse()) {
			if (this->GetCredits() < pItem->GetPersonalStorePrice()) {
				pMsg.result = 2;
				this->SendPacket(&pMsg);
				return;
			}
			...
			if (this->MoneyGet() < pItem->GetPersonalStorePrice()) {
				pMsg.result = 2;
				this->SendPacket(&pMsg);
				return;
			}
	...
	if (pItem->GetPersonalStorePrice() > 0)
	{
		if (sGameServer->IsPersonalStoreCreditsPurcharse()) {
			...
			this->MoneyReduce(pItem->GetPersonalStorePrice());
			pPlayer->MoneyAdd(pItem->GetPersonalStorePrice());
		}
	}
...
void Player::HandlePersonalStoreSellList(uint8 * Packet)
{
	...
	head->Set(HEADCODE_PERSONAL_STORE, SUBCODE_PERSONAL_STORE_SELL_LIST, sizeof(PERSONAL_STORE_SELL_LIST_HEAD)+(sizeof(PERSONAL_STORE_SELL_LIST_BODY)* head->count));
	this->sendPacket(buffer);
}
...
void Player::HandlePersonalStoreSellItemView(uint8 * Packet)
{
	...
	head->Set(HEADCODE_PERSONAL_STORE, SUBCODE_PERSONAL_STORE_SELL_ITEM_VIEW, sizeof(PERSONAL_STORE_SELL_ITEM_VIEW_HEAD)+(sizeof(PERSONAL_STORE_SELL_ITEM_VIEW_BODY)* head->count));
	this->sendPacket(buffer);
...
void Player::SendPersonalStoreList(uint32 page, uint32 data)
{
	...
	head->Set(HEADCODE_PERSONAL_STORE, SUBCODE_PERSONAL_STORE_LIST, sizeof(PERSONAL_STORE_SEARCH_HEAD)+(sizeof(PERSONAL_STORE_SEARCH_BODY)* head->count));
	this->sendPacket(buffer);
...
void Player::SendPersonalStoreItemList(uint32 page, uint32 data, uint16 item, std::string const& item_name)
{
	...
	head->Set(HEADCODE_PERSONAL_STORE, SUBCODE_PERSONAL_STORE_ITEM_LIST, sizeof(PERSONAL_STORE_SEARCH_ITEM_LIST_HEAD)+(sizeof(PERSONAL_STORE_SEARCH_ITEM_LIST_BODY)* head->count));
	this->sendPacket(buffer);
