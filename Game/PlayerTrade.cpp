...
void Player::TradeBegin(Player* pPlayer01, Player* pPlayer02)
{
	...
	if ( pPlayer01->GetInterfaceState()->GetType() == 1 )
	{
	...
	}else if(pPlayer01->GetBetItemData(BET_ITEM_MAX)->IsDuelBetTradeAccept() && pPlayer02->GetBetItemData(BET_ITEM_MAX)->IsDuelBetTradeAccept()) {
		pPlayer01->GetInterfaceState()->Set(InterfaceData::Trade, pPlayer02, 0, sGameServer->trade_max_wait_interval.get());
		pPlayer01->GetInterfaceState()->SetType(1);
		...
		pPlayer01->GetBetItemData(BET_ITEM_MAX)->SetDuelBeginTrade(true);
		pPlayer02->GetBetItemData(BET_ITEM_MAX)->SetDuelBeginTrade(true);
...
void Player::TradeOK(Player* pPlayer01, Player* pPlayer02)
{
	...
	if (pPlayer01->GetBetItemData(BET_ITEM_MAX)->IsDuelBeginTrade() && pPlayer02->GetBetItemData(BET_ITEM_MAX)->IsDuelBeginTrade()) {

		pPlayer01->MoneySet(pPlayer01->MoneyGet() - pPlayer01->GetTradeInventory()->GetZen());
		pPlayer02->MoneySet(pPlayer02->MoneyGet() - pPlayer02->GetTradeInventory()->GetZen());

		pPlayer01->GetBetItemData(BET_ITEM_MAX)->SetBetZen(pPlayer01->GetTradeInventory()->GetZen());
		pPlayer02->GetBetItemData(BET_ITEM_MAX)->SetBetZen(pPlayer02->GetTradeInventory()->GetZen());
		...
		pPlayer01->GetBetItemData(BET_ITEM_MAX)->SetDuelFinishTrade(true);
		pPlayer02->GetBetItemData(BET_ITEM_MAX)->SetDuelFinishTrade(true);

		sDuelBetItem->DuelBegin(pPlayer01, pPlayer02);

		//pPlayer01->SendMessageBox(0, "Test", "Bet Zen: %d", pPlayer01->GetBetItemData(BET_ITEM_MAX)->GetBetZen());
	}
	else {
		...
		pPlayer01->SaveDBAllItemBox();
		pPlayer02->SaveDBAllItemBox();
...
bool Player::TradeToInventory()
{
	...
	if (!this->GetBetItemData(BET_ITEM_MAX)->IsDuelBeginTrade()) {
		trade_loop(i)
		{
...
{
	pPlayer01->GetBetItemData(BET_ITEM_MAX)->Reset();
	pPlayer02->GetBetItemData(BET_ITEM_MAX)->Reset();
