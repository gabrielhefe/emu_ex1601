...
void ProtectorOfAcheron::LoadIni() {
	if (boost::filesystem::exists(ARCHERON_FILEPATH)) {
		sGameServer->SetProtectorOfAcheronEnabled(GetPrivateProfileInt(ARCHERON_SECTION, "Enabled", 0, ARCHERON_FILEPATH));
	}
	else {
		sLog->outError(LOG_DEFAULT, "Error Load Archeron.ini File");
		MessageBox(NULL, "Error Load Archeron.ini File", "Error", ERROR);
		exit(0);
	}
}
...
void ProtectorOfAcheron::Load()
{
	...
	ReadScript file("../Data/Event/EventData/ProtectorOfAcheron.txt", "rb");
