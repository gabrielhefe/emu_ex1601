...
void Raklion::LoadIni() {
	if (boost::filesystem::exists(RANKLION_FILEPATH)) {
		sGameServer->SetRaklionEnabled(GetPrivateProfileInt(RANKLION_SECTION, "Enabled", 0, RANKLION_FILEPATH));
		sGameServer->SetRaklionFailTimeMax(GetPrivateProfileInt(RANKLION_SECTION, "FailTimeMax", 0, RANKLION_FILEPATH) * MINUTE * IN_MILLISECONDS);
		sGameServer->SetRaklionFailTimeMin(GetPrivateProfileInt(RANKLION_SECTION, "FailTimeMin", 0, RANKLION_FILEPATH) * MINUTE * IN_MILLISECONDS);
		sGameServer->SetRaklionSuccessTimeMax(GetPrivateProfileInt(RANKLION_SECTION, "SuccessTimeMax", 0, RANKLION_FILEPATH) * MINUTE * IN_MILLISECONDS);
		sGameServer->SetRaklionSuccessTimeMin(GetPrivateProfileInt(RANKLION_SECTION, "SuccessTimeMin", 0, RANKLION_FILEPATH) * MINUTE * IN_MILLISECONDS);

	}
	else {
		sLog->outError(LOG_DEFAULT, "Error Load Raklion.ini File");
		MessageBox(NULL, "Error Load Raklion.ini File", "Error", ERROR);
		exit(0);
	}
}
...
void Raklion::LoadData()
{
	...
	std::string filename("../Data/Event/EventData/Raklion.txt");
