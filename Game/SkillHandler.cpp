...
{
	int32 value_damage = (this->GetCaster()->GetEnergy() / 70);
	//int32 value_defense = (this->GetCaster()->GetEnergy() / 60);
	int32 value_defense = 0;
	int32 value_speed = (this->GetCaster()->GetEnergy() / 200) + this->GetCaster()->MagicGetValue(470);
	int32 duration = WEEK;

	if (this->GetSkill()->GetSkill() == 470)
	{
		value_defense = sGameServer->GetSkillBerserkerlDecHP() * 60 / 100;
	}
	else {
		value_defense = sGameServer->GetSkillBerserkerlDecHP();
	}
...
void SkillHandler::SkillBuffDarkness()
{
	...
	int32 value_decrease = sGameServer->GetSkillDarkNessDecHP();
	int32 duration = WEEK;
...
void SkillHandler::SkillDetection()
{
	...
	head->Set(HEADCODE_MINIMAP_DATA, SUBCODE_MINIMAP_DATA_SKILL_DETECTION, sizeof(UNIT_DETECTION_HEAD)+(head->count * sizeof(UNIT_DETECTION_BODY)));
...
void SkillHandler::SkillDemolish()
{
	...
	/*uint8 magictreelevel = this->GetCaster()->GetMagicTreeLevel(788);

	if (magictreelevel == 1) {
		damage_reduction += 20;
	}
	else if (magictreelevel == 2) {
		damage_reduction += 33;
	}
	else if (magictreelevel == 3) {
		damage_reduction += 45;
	}
	else if (magictreelevel == 4) {
		damage_reduction += 57;
	}
	else if (magictreelevel == 5) {
		damage_reduction += 68;
	}
	else if (magictreelevel == 6) {
		damage_reduction += 77;
	}
	else if (magictreelevel == 7) {
		damage_reduction += 86;
	}
	else if (magictreelevel == 8) {
		damage_reduction += 95;
	}
	else if (magictreelevel == 9) {
		damage_reduction += 102;
	}
	else if (magictreelevel == 10) {
		damage_reduction += 109;
	}
	else if (magictreelevel == 11) {
		damage_reduction += 115;
	}
	else if (magictreelevel == 12) {
		damage_reduction += 121;
	}
	else if (magictreelevel == 13) {
		damage_reduction += 126;
	}
	else if (magictreelevel == 14) {
		damage_reduction += 130;
	}
	else if (magictreelevel == 15) {
		damage_reduction += 134;
	}
	else if (magictreelevel == 16) {
		damage_reduction += 138;
	}
	else if (magictreelevel == 17) {
		damage_reduction += 141;
	}
	else if (magictreelevel == 18) {
		damage_reduction += 143;
	}
	else if (magictreelevel == 19) {
		damage_reduction += 145;
	}
	else if (magictreelevel == 20) {
		damage_reduction += 147;
	}*/
	...
	for (int32 i = 0; i < this->party_member_count; ++i)
	{
		...
		{
			this->PartyMember[i]->AddBuff(this->GetSkillInfo()->GetBuffIcon(),
				BuffEffect(BUFF_OPTION_IGNORE_BASE_DEFENSE, defense_ignore),
				BuffEffect(BUFF_OPTION_DEMOLISH_DAMAGE_DECREASE, damage_reduction), duration, 0, this->GetCaster(), true);
		}
	}
