...
void StoreScript::LoadDBItem(const char* ItemArray, uint32 owner, ItemBox box)
{
	Tokenizer tokens(ItemArray, ';');
	std::string slotStr = tokens[1];
	uint8 slot = atoi(slotStr.c_str());

	this->item[slot].LoadDB(ItemArray, owner, slot, box);
...
void StoreScript::LoadDBItemNew(const char* ItemArray, uint32 owner, ItemBox box)
{
	Tokenizer tokens(ItemArray, ';');
	std::string slotStr = tokens[0];
	uint8 slot = atoi(slotStr.c_str());

	this->item[slot].LoadDBNew(ItemArray, owner, slot, box);
