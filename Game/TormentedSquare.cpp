...
void TormentedSquare::LoadIni() {
	if (boost::filesystem::exists(TORMENTEDSQUARE_FILEPATH)) {
		sGameServer->SetTormentedSquareEnabled(GetPrivateProfileInt(TORMENTEDSQUARE_SECTION, "Enabled", 0, TORMENTEDSQUARE_FILEPATH));
		sGameServer->SetTormentedSquareSurvivalEnabled(GetPrivateProfileInt(TORMENTEDSQUARE_SECTION, "MonsterSpeed", 0, TORMENTEDSQUARE_FILEPATH));
	}
	else {
		sLog->outError(LOG_DEFAULT, "Error Load TormentedSquare.ini File");
		MessageBox(NULL, "Error Load TormentedSquare.ini File", "Error", ERROR);
		exit(0);
	}

}

void TormentedSquare::Load(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EventTormentedSquareStage");

	uint32 count = 0;

	for (pugi::xml_node eventtor = Main.child("EventTor"); eventtor; eventtor = eventtor.next_sibling()) {
		for (pugi::xml_node data = eventtor.child("Data"); data; data = data.next_sibling()) {
			uint8 ground = eventtor.attribute("Ground").as_int();
			uint8 stage = data.attribute("Stage").as_int();

			if (ground < MAX_TORMENTED_SQUARE_GROUND && stage < MAX_TORMENTED_SQUARE_ROUND)
			{
				this->GetGround(ground)->GetStageData(stage)->SetStandBy(data.attribute("Standby").as_int());
				this->GetGround(ground)->GetStageData(stage)->SetDuration(data.attribute("Duration").as_int());
...
void TormentedSquare::LoadRanking(char* pchFileName)
{
	...
	pugi::xml_node Main = file.child("EventTormentedSquareRanking");
	...
	for (pugi::xml_node eventtor = Main.child("EventTor"); eventtor; eventtor = eventtor.next_sibling()) {
		for (pugi::xml_node data = eventtor.child("Data"); data; data = data.next_sibling()) {
			uint8 ground = eventtor.attribute("Ground").as_int();
			uint8 rank = data.attribute("Rank").as_int();

			if (ground < MAX_TORMENTED_SQUARE_GROUND && rank < TORMENTED_SQUARE_MAX_TEAM)
			{
				this->GetGround(ground)->SetReward(rank, data.attribute("Reward").as_int());
