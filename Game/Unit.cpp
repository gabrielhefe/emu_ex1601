...
void Unit::AttackProc(Unit * pTarget, Skill * mSkill, bool magic_send, int32 damage, uint8 combo, uint16 damage_type, int32 count)
{
	...
	if ( damage == 0 )
	{
		...
		{
			if (sGameServer->IsSkillAddDamageEnabled()) {
				if (Player* pPlayer = this->ToPlayer())
				{
					if (pPlayer->GetClass() == Character::DARK_KNIGHT)
					{
						...
						{
							damage = damage * (50 + (this->GetEnergy() / 15)) / 100;
						}
						else
						{
							damage = damage * (165 + (this->GetEnergy() / 15)) / 100;
						}
					}
		...
		case SKILL_EVIL_WIND:
			{
			if (sGameServer->IsSkillAddDamageEnabled()) {
				damage = damage * (150 + (this->GetEnergy() / 25)) / 100;
			}
			} break;
		...
		case SKILL_SPIRIT_HOOK:
			{
				damage = (damage + this->GetVitality() / 20) * (this->GetVitality() / 20 + 35) / 100;
			} break;
		...
		case SKILL_CHARGE:
			{
				damage = damage * (this->GetVitality() / 20 + 50) / 100;
			} break;
		...
		case SKILL_DARK_SIDE:
			{
				damage = damage * (this->GetAgility() / 20 + 25) / 100;
			} break;

		case SKILL_DRAGON_LORE:
			{
				damage = (damage + this->GetEnergy() / 25) * (this->GetEnergy() / 25 + 50) / 100;
			} break;
		...
		case SKILL_DRAGON_SLAYER:
			{
				if (pTarget->IsPlayer())
				{
					damage = damage * (this->GetEnergy() / 25 + 50) / 100;
				}
				else
				{
					damage = (damage * (this->GetEnergy() / 25 + 50) / 100 + 50) * 2;
				}
				//damage = (damage * (this->GetEnergy() / 15 + 50) / 100 + 100 ) * 2;
			} break;

		case SKILL_PHOENIX_SHOT:
			{
				damage = damage * (this->GetVitality() / 20 + 135) / 100;
			} break;

		case SKILL_DARK_PHOENIX_SHOT:
			{
				damage = damage * ((this->GetAgility() / 20) + 50) / 100;
			} break;
		...
		case SKILL_MAGIC_ARROW:
			{
				//damage = damage * (this->GetEnergy() / 10 + 150) / 100;
		...
		case SKILL_BAT_FLOCK:
			{
			if (sGameServer->IsSkillAddDamageEnabled()) {
				damage = damage * ((this->GetStrength() / 15) + (this->GetAgility() / 25) + 120) / 100;
			}
			...
			if (skill != 0)
			{
				...
				if (pSkillInfo)
				{
					if (pTarget->IsCreature())
					{
						...
						{
							if (sGameServer->GetDamagePlayerAddPercent() != 0) {
								damage += damage * sGameServer->GetDamagePlayerAddPercent() / 100;
							}
					...
					else if (pTarget->IsPlayer())
					{
						if (pSkillInfo->GetPVPDamage() != 0)
						{
							...
							Player* playerCaster = this->ToPlayer();
							Player* playerTarget = pTarget->ToPlayer();
							damage = sCharacterBase->GetCalcPVPDamage(playerCaster->GetClass(), playerTarget->GetClass(), damage);
						}
					}
				}
			}
		...
		if ( this->IsPlayer() )
		{
			if ( pVictim->IsPlayer() )
			{
				...
				if (pVictim->PowerGet(POWER_SHIELD) > 0) {
					pVictim->PowerReduce(POWER_SHIELD, tmp_shield_damage * sGameServer->GetDamageToSDPersent() / 100, true);
				}
				else {
...
void Unit::PowerIncreseAdd(uint8 id, int32 value, bool compare)
{
	...
	if (compare)
	{
		this->PowerSet(id, this->PowerGet(id), true);
	}
}
...
bool Unit::AttackElemental(Unit* pTarget, Skill* pSkill, bool magic_send, int32 damage, int32 count, bool combo)
{
	...
	if (pPlayer && pVictim && count <= 1)
	{
		...
		{
			pVictim->AddBuff(BUFF_ERRTEL_BERSERKER_I, pVictim->GetPentagramJewelOption()->GetElementalSkillBerserkerI(), 0, pVictim);
		}
	}
