...
void WorldBoss::CreateBoss()
{
	...
	if (random_monster)
	{
		...
		if (monster)
		{
			...
			monster->SetDespawnTime(3600 * IN_MILLISECONDS);
			monster->SetDespawnTick(MyGetTickCount());
			monster->SetDespawnType(MONSTER_DESPAWN_TIME);
			...
			case 1:
				...
				sLog->outInfo(LOG_DEFAULT, "World Boss Spawn In %s X: %d - Y: %d", monster->GetWorldName(), monster->GetX(), monster->GetY());
				break;
...
std::string WorldBoss::GenerateItemBag() const
{
	...
	if (ItemBag const* pItemBag = sItemBagMgr->GetItemBag(sGameServer->GetWorldBossItemBag().c_str()))
