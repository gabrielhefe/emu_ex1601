...
void WorldSocket::Start()
{
	...
	QueryResult result = MuDatabase.PQuery("SELECT COUNT(*) FROM account_data WHERE current_ip='%s' and current_server!=65535", GetRemoteIpAddress().to_string().c_str());

	if (result) {
		Field* reader = result->Fetch();
