...
void AuthSocket::AccountReconnect(uint8 * Packet)
{
	...
	sLoginQueue->ConnectAccount(lpMsg->start_server, lpMsg->h.server, lpMsg->account, lpMsg->account_id, lpMsg->security_code, lpMsg->ip, lpMsg->mac, lpMsg->facebook_status, lpMsg->golden_channel, lpMsg->disk_serial, lpMsg->authorized, 0);
}
...
void AuthSocket::AccountServerMove(uint8 * Packet)
{
	...
	if ( !pAccountData )
	{
		...
		QueryResult result = LoginDatabase.PQuery("SELECT a.current_server, b.account, b.guid, b.security_code, a.last_ip, a.last_mac, a.disk_serial, a.type From accounts_status AS a LEFT JOIN accounts AS b ON a.account_id = b.guid WHERE a.account_id=%d", lpMsg->data.account_id);

		//ConnectAccount(uint16 start_server, uint16 current_server, const char* account, uint32 guid, const char* secure_code, const char* ip, const char* mac, uint8 facebook_status, time_t golden_channel, uint32 disk_serial, bool authorized, uint8 acctype)
		if (result) {
			Field* field = result->Fetch();
			sLoginQueue->ConnectAccount(field[0].GetUInt32(), field[0].GetUInt32(), field[1].GetString().c_str(), field[2].GetUInt32(), field[3].GetString().c_str(), field[4].GetString().c_str()
				, field[5].GetString().c_str(), 0, 0, field[5].GetUInt32(), 0, field[6].GetUInt16());
			pAccountData = sLoginQueue->GetAccountData(lpMsg->h.GetServerGroup(), lpMsg->data.account_id);
		}
		else {
			pMsg.result = 3;
			this->SendPacket((uint8*)&pMsg, pMsg.h.size);
